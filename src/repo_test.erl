...
% repo_test:create_user_test("13", 1, 10).
% repo_test:create_user_test("15", 1, 500000).
...
% repo_test:create_friend_test(2, 3).
% repo_test:create_friend_test(13335).
...
create_friend_test(FromId, ToId) when FromId == ToId ->
    ok;
% repo_test:create_friend_test(513238, 1).
% repo_test:create_friend_test(1, 513238).
% repo_test:create_friend_test(513238, 513238).
...
create_friend_test(FromId, ToId) ->
    ...
    mysql_pool:query(Sql, [FromId, ToId, util_dt:milliseconds()]).
...
% repo_test:test_use_catch()
test_use_catch() ->
    [{I, catch generate_exception(I)} || I <- lists:seq(1, 5)].


% repo_test:test_user_try_catch()
test_user_try_catch() ->
    [begin
         try
