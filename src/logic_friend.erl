...
move_to_category(CurrentUid, Uid, CategoryId) ->
    repo_friend:move_to_category(CurrentUid, Uid, CategoryId),
    ok.
    ...
    case ds_friend:find_by_uid(Uid, Column) of
        ...
        Friends ->
            ...
            Users2 = [replace_remark({Id, Row}, FriendRemark, false) ||
                         [{<<"id">>, Id} | _] = Row <- Users],
            % 获取用户在线状态
            [ds_user:online_state(User) || {_Id, User} <- Users2]
    end.
    ...
    case ds_friend:find_by_uid(Uid, Column) of
        ...
        Friends ->
            ...
            Users2 = [replace_remark({Id, Row}, FriendRemark, true) ||
                         [{<<"id">>, Id} | _] = Row <- Users],
            % 获取用户在线状态
            Users3 = [{Id, ds_user:online_state(User)} ||
                         {Id, User} <- Users2],
            % 把用户归并到相应的分组
            Groups = ds_friend_category:find_by_uid(Uid),
