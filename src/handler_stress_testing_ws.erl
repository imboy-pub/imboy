-module(handler_stress_testing_ws).
...
init(Req0, State0) ->
    ...
    case cowboy_req:match_qs([{token, [], undefined}], Req0) of
        ...
        #{token := Token} ->
            ...
            case catch ds_token:decrypt_token(Token) of
...
websocket_init(State) ->
    ...
    case lists:keyfind(error, 1, State) of
        ...
        false ->
            ...
            % 用户上线
            logic_user:online(CurrentUid, CurrentPid, <<"web">>),
            {ok, State, hibernate}
    end.
        ...
        case lists:keyfind(error, 1, State) of
            ...
            false ->
                ...
                case cowboy_bstr:to_upper(Type) of
                    <<"C2C">> ->
                        logic_websocket:dialog(CurrentUid, Data);

                    <<"GROUP">> ->
                        logic_websocket:group_dialog(CurrentUid, Data);

                    <<"S2C">> ->
                        logic_websocket:system(CurrentUid, Data)
                end
        end
    of
        Res ->
