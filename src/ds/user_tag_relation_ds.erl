...
%% @doc 保存用户标签关系
-spec save_user_tag_relation(pid(), integer(), integer(), integer(), binary(), binary()) ->
    integer().
...
%% @doc 查询标签
-spec select_tag(binary(), list(), binary()) ->
    {ok, non_neg_integer(), list(map())} | {error, term()}.
select_tag(Where, WhereArgs, Column) ->
    user_tag_relation_repo:select_tag(Where, WhereArgs, Column).

%% @doc 移除用户标签关系
-spec remove_user_tag_relation(
    pid(), binary(), binary() | integer(), integer(), binary() | integer()
) -> ok.
...
%% @doc 替换对象标签
-spec replace_object_tag(pid(), binary(), binary() | integer(), binary() | integer(), [binary()], [
    binary()
]) -> ok.
...
%% @doc 保存标签
-spec save_tag(pid(), integer(), integer(), binary(), binary()) ->
...
%% @doc 更新标签
-spec update_tag(pid(), integer(), binary(), integer(), binary()) ->
    {integer(), binary()} | {error, term()}.
