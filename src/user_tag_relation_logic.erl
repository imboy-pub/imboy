...
set(Uid, Scene, ObjectIds, TagId, TagName) ->
    ...
    Check = imboy_db:pluck(
        <<"user_tag">>
        , <<"scene = ", Scene/binary, " AND creator_user_id = ", Uid2/binary, " AND name = '", TagName/binary,"' AND id != ", TagId/binary>>
        , <<"count(*)">>
        , 0
    ),
    if
...
do_add(Scene, Uid, ObjectId, Tag) ->
    ...
    imboy_db:with_transaction(fun(Conn) ->
        ...
        [
            user_tag_logic:change_scene_tag(Conn, Scene, Uid2, ObjectId, [{integer_to_binary(TagId), N}]) || {TagId, N} <- TagIdNewLi, TagId > 0
        ],
