-module(repo_verification_code).
%%%
% repo_verification_code 是 verification_code repository 缩写
%%%
...
-spec get_by_id(Id :: binary) ->
          {ok, ColumnList :: list(), Values :: list()}.
get_by_id(Id) ->
    Sql = <<"SELECT `id`,`code`,`validity_at`,
        `created_at` FROM `verification_code` WHERE `id` = ?">>,
    Row = mysql_pool:query(Sql, [Id]),
    % lager:info("~p", Row),
    Row.


% repo_verification_code:save(<<"test@imboy.pub">>,
%   util_func:num_random(6),
%   util_dt:milliseconds() + 600000,
