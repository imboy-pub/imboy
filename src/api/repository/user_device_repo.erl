...
login_count(UID, DID) ->
    Sql = <<"SELECT `login_count` FROM `user_device` WHERE `status` = ? AND `user_id` = ? AND `device_id` = ?">>,
    case mysql_pool:query(Sql, [1, UID, DID]) of
        {ok, _FieldList, [[LoginCount]]} when is_integer(LoginCount), LoginCount > 0 ->
            LoginCount;
        _ ->
            0
    end.

-spec save(Now::integer(), UID::integer(), DID :: binary(), PostVals::list()) -> ok.
...
-spec save(Now::integer(), UID::integer(), PostVals::list(), DID::binary(), LoginCount::integer()) -> ok.
save(Now, UID, PostVals, DID, LoginCount) when bit_size(DID) > 0, LoginCount > 0 ->
    % 更新登录次数，最近登录时间、IP
    Ip = proplists:get_value(<<"ip">>, PostVals, <<"">>),
    Sql = <<"UPDATE `user_device` SET `login_count` = ?, `last_login_ip` = ?,`last_login_at` = ? WHERE `status` = ? AND `user_id` = ? AND `device_id` = ?">>,
    mysql_pool:execute(Sql, [LoginCount + 1, Ip, Now, <<"1">>, UID, DID]);
save(Now, UID, PostVals, DID, _LoginCount) when bit_size(DID) > 0 ->
    ...
    Table = <<"`user_device`">>,
    Column = <<"(`user_id`,`device_type`,`device_id`,`device_vsn`,`device_name`,`login_count`,`last_login_ip`,`last_login_at`,`status`,`created_at`)">>,
    Value = <<"('",
        UID2/binary, "', '",
        DeviceType/binary, "', '",
        DID/binary, "', '",
