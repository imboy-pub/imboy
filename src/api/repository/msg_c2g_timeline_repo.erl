...
find_by_uid(Uid, Column, Limit) ->
    ...
    Sql = <<"SELECT ", Column/binary,
        " FROM `msg_c2g_timeline`",
        Where/binary>>,
    mysql_pool:query(Sql, [Uid, Limit]).

delete_timeline(Id) ->
    Where = <<"WHERE `id` = ?">>,
    Sql = <<"DELETE FROM `msg_c2g_timeline` ",
        Where/binary>>,
    mysql_pool:query(Sql, [Id]).

check_msg(MsgId) ->
    Sql = <<"SELECT count(*) as count FROM `msg_c2g_timeline` WHERE `msg_id` = ?">>,
    mysql_pool:query(Sql, [MsgId]).

count_by_to_id(ToUid) ->
    % use index i_ToId
    Sql = <<"SELECT count(*) as count FROM `msg_c2g_timeline` WHERE `to_id` = ?">>,
    mysql_pool:query(Sql, [ToUid]).

delete_overflow_timeline(ToUid, Limit) ->
    % use index i_ToId
    Sql = <<"SELECT `id`,`msg_id` FROM `msg_c2g_timeline` WHERE `to_id` = ? ORDER BY `id` ASC LIMIT ?">>,
