...
page(<<"GET">>, Req0, State) ->
    ...
    case Gid2 of
        ...
        _ ->
            ...
            Column = <<"id as notice_id, user_id, edit_user_id, body, status, expired_at, updated_at, created_at">>,
            % 使用安全的参数化查询，避免SQL注入
            Where = #{group_id => Gid2},
            Tb = group_notice_repo:tablename(),
            {ok, Payload} = imboy_pg:page_with_total(
                Tb,
                Column,
                Where,
                <<"expired_at desc">>,
...
latest(<<"GET">>, Req0, State) ->
    ...
    case Gid2 of
        ...
        _ ->
            ...
            Tb = group_notice_repo:tablename(),
            Sql = <<"SELECT ", Column/binary, " FROM ", Tb/binary, " WHERE ", Where/binary, " ORDER BY id desc">>,
