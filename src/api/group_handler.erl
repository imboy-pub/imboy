...
face2face_save(Req0, State) ->
    ...
    case group_logic:face2face_save(Code, Gid2, Uid) of
            {ok, _} ->
                {ok, MemberList} = group_member_logic:list_member(Gid2),
                case group_repo:find_by_id(Gid2, <<"*">>) of
                    {error, Reason} ->
...
add(Req0, State) ->
    ...
    case throttle:check(three_second_once, Uid) of
        {limit_exceeded, _, _} ->
            imboy_response:error(Req0, "在处理中，请稍后重试");
        _ ->
            Count = imboy_pg:pluck_value(group_repo:tablename(), <<"count(*)">>, #{status => 1, owner_uid => Uid}, 0),
            PostVals = imboy_param:post(Req0),
...
msg_page(Req0, State) ->
    ...
    GMSize = maps:size(GM),
    WhereSql = case imboy_param:int(last_time, Req0, 0) of
        {ok, Last} when Last > 0 ->
            ...
            case group_repo:find_by_id(Gid2, Column) of
                ...
                G ->
                    Res = group_member_logic:join_group(
                        ...
                        Gid2,
                        #{
                            max_members => maps:get(<<"member_max">>, G, 10),
                            current_count => maps:get(<<"member_count">>, G, 0)
                        }
                    ),
