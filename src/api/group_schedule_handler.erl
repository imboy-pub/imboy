...
create(Req0, State) ->
    ...
    StartAt = maps_get(<<"start_at">>, PostVals),
    EndAt = maps_get(<<"end_at">>, PostVals),
    RemindBefore = maps:get(<<"remind_before">>, PostVals, 15),
    ParticipantIds0 = maps:get(<<"participant_ids">>, PostVals, []),

    % 解码 HashID
    GroupId2 = elib_hashids:decode(GroupId),
    ParticipantIds = [elib_hashids:decode(Id) || Id <- ParticipantIds0],
...
update(Req0, State) ->
    ...
    ScheduleId = maps_get(<<"schedule_id">>, PostVals),
...
detail(Req0, _State) ->
    ...
    ScheduleId = proplists:get_value(<<"schedule_id">>, Qs, undefined),
...
list(Req0, _State) ->
    ...
    case GroupId2 of
        0 ->
            elib_response:error(Req0, error_msg(?ERR_BAD_REQUEST), ?ERR_BAD_REQUEST);
        _ ->
            case group_schedule_logic:list_group_schedules(GroupId2, Page, Size) of
                {ok, Result} ->
                    elib_response:success(Req0, Result);
                {error, Reason} ->
...
my_list(Req0, State) ->
    ...
    case group_schedule_logic:list_my_schedules(Uid, Page, Size) of
        {ok, List} ->
