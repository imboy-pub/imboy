...
apply_logout(Uid, Req0) ->
    ...
    imboy_pg:with_tx(fun(Conn) ->
        Now = imboy_dt:now(),
        % 事务中使用Conn参数
        {ok, _} = imboy_pg:update(Conn, user_repo:tablename(),
            #{
...
cancel_logout(Uid, _Req0) ->
    ...
    imboy_pg:update(user_repo:tablename(),
        #{
            % 状态: -1 删除  0 禁用  1 启用  2 申请注销中
            <<"status">> => 1
        }, <<"id = $1">>, [Uid]),
...
-spec find_by_ids(any()) -> any().
find_by_ids(Ids) ->
    find_by_ids(Ids, ?DEF_USER_COLUMN).


-spec find_by_ids(any(), any()) -> any().
find_by_ids([], _) ->
    [];
...
update(Uid, <<"gender">>, Val) when Val =:= <<"1">>; Val =:= <<"2">>; Val =:= <<"3">> ->
    do_update_field(user_repo:tablename(), Uid, <<"gender">>, Val);
update(Uid, <<"allow_search">>, Val) when Val =:= <<"1">>; Val =:= <<"2">> ->
    do_update_field(fts_user_repo:tablename(), Uid, <<"allow_search">>, Val, <<"user_id = $1">>);
...
do_update_field(Table, Uid, Field, Val, WhereSql) ->
    imboy_pg:update(Table, #{Field => Val}, WhereSql, [Uid]).
...
send_bind_email(Uid, Email) ->
    ...
    Body = <<"Hi, ", Title/binary,
        ...
        ExpireAt/binary, "前链接有效。<br/>如果你没有操作绑定此邮箱，请忽略此邮件。<br/><br/> 如果需要了解更多信息，请访问IMBoy官方网站：https://www.imboy.pub/"/utf8>>,
