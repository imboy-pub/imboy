    ...
    case nearby_gid(LngFloat, LatFloat, <<"50">>, <<"m">>, <<"1">>, Code) of
        {ok, []} ->
            imboy_pg:with_tx(fun(Conn) ->
                ...
                % 事务中需要使用Conn参数
                %% INSERT 参数化
                {Sql, Params} = imboy_pg_sql:insert_with_params(
        ...
        {ok, [#{<<"group_id">> := Gid}]} ->
            ...
            group_member_logic:join_group(JoinMode, Uid, Gid, #{}),
            {ok, Gid};
        _ ->
            {error, "error"}
    end.
...
-spec face2face_save(binary(), any(), any()) -> ok | {error, binary()}.
face2face_save(Code, Gid, Uid) ->
    imboy_pg:with_tx(fun(Conn) ->
        %% 1. 读取随机码记录
        RowCode = case group_random_code_repo:find_by_gid(Gid, <<"code,user_id">>) of
            #{<<"code">> := RC} ->
                RC;
            _ ->
                throw({abort_tx, <<"gid not exist">>})
        end,
...
add(_, Uid, Type, MemberUids) ->
    ...
    % 使用安全的参数化查询，避免SQL注入
    Sql = <<"SELECT id FROM ", (group_repo:tablename())/binary, " WHERE creator_uid = $1 AND user_id_sum = $2">>,
    GidOld = case imboy_pg:query(Sql, [Uid, Sum]) of
        {ok, [#{<<"id">> := Gid0}]} ->
            Gid0;
        _ ->
            0
    ...
    case GidOld of
        0 ->
            ...
            imboy_pg:with_tx(fun(Conn) ->
                ...
                group_ds:join(Uid, Gid),
                [group_member_logic:join_group(Conn, JoinMode, Uid2, Gid, #{}) || Uid2 <- MemberUids2, Uid2 /= Uid],
                {ok, Gid}
            end);
...
create_group(Conn, Gid, Uid, Now, Type, JoinLimit) ->
    ...
    Gid2 = case imboy_pg_sql:parse_result(imboy_pg:insert(Conn, group_repo:tablename(), GMap2, <<"RETURNING id">>)) of
        {ok, Id, _} -> Id;
        _ -> Gid
    end,
    % 检查群成员是否已存在，不存在则插入
    case group_member_repo:find(Gid2, Uid, <<"id">>) of
        GM when map_size(GM) == 0 ->
            % 事务中插入群成员
            imboy_pg:insert(Conn, group_member_repo:tablename(), #{
                group_id => Gid2,
                user_id => Uid,
