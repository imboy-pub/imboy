...
-spec people_nearby(Lng :: binary(), Lat :: binary(), Radius :: integer(), Unit :: binary(), Limit :: integer()) ->
          list().
people_nearby(Lng, Lat, Radius, <<"km">>, Limit) when is_binary(Radius) ->
    RadiusM = binary_to_integer(Radius) * 1000,
    people_nearby(Lng, Lat, RadiusM, <<"m">>, Limit);
people_nearby(Lng, Lat, Radius, Unit, Limit) ->
    ...
    [ imboy_hashids:replace_id(Row) || Row <- Li ].
