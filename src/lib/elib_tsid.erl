...
-export([init/1, generate/0, generate_n/1]).
...
%% persistent_term 键
-define(PT_STATE,   elib_tsid_state).
...
init(Opts) ->
    ...
    %% 用 atomics 存储状态 (lock-free)
    %% 状态编码: (RelativeTimestamp bsl SEQUENCE_BITS) bor Sequence
    %% 这样状态值天然单调递增，CAS 操作安全
    StateRef = atomics:new(1, [{signed, true}]),

    persistent_term:put(?PT_STATE,   StateRef),
