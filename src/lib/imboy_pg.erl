...
pluck(Table, Field, Where, Opts, Default) ->
    Fields = [Field],
    {Sql, Params} =
        imboy_pg_sql:build_select(Table, Fields, Where, Opts#{limit => 1}),
...
insert(Table, Map, RETURNING) ->
    {Sql, Params} = imboy_pg_sql:insert(Table, Map, RETURNING),
    execute(Sql, Params).
...
%%
%% 示例：
%%   imboy_pg:update(<<"user">>, #{name => "Alice", age => 30},
...
%%       imboy_pg:update(Conn, <<"user">>, #{name => "Alice", age => 30},
%%                       <<"id = $1 AND status = $2">>, [UserId, Status])
%%   end)
%%
%% 安全性：
...
page(Table, Column, WhereMap, OrderBy, Page, Size) when Page > 0, Size > 0 ->
    Limit = Size,
    Offset = (Page - 1) * Size,
    {Sql, Params} = imboy_pg_sql:page(Table, Column, WhereMap, OrderBy, Limit, Offset),
    query(Sql, Params).
...
%%       imboy_pg:page_with_total(Tb, Column, Where, OrderBy, Page, Size)
%%--------------------------------------------------------------------
