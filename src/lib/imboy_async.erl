...
%% 示例：
%% ```
%% imboy_async:async_retry(Fun, 5).
%% ```
-spec async_retry(fun(), non_neg_integer()) -> pid().
...
%% 示例：
%% ```
%% imboy_async:async_retry(Fun, 3, 1000, exponential).
%% ```
-spec async_retry(fun(), non_neg_integer(), non_neg_integer()) -> pid().
async_retry(Fun, RetryCount, DelayMs) ->
    spawn(fun() ->
        case imboy_retry:with_retry(Fun, RetryCount, DelayMs) of
            {ok, _Result} ->
                ok;
            {error, Reason} ->
...
%% 示例：
%% ```
%% imboy_async:async_with_timeout(Fun, 5000).
%% ```
-spec async_with_timeout(fun(), timeout()) -> pid().
async_with_timeout(Fun, TimeoutMs) ->
    spawn(fun() ->
        case imboy_retry:with_retry_and_timeout(Fun, TimeoutMs, 3) of
            {ok, Result} ->
                {ok, Result};
            {error, Reason} ->
