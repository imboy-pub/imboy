...
handle_cast({login_success, UID, PostVals}, State) ->
    ...
    repo_user_device:save(Now, UID2, DID, PostVals),
    % 记录设备信息 END
    {noreply, State, hibernate};
...
handle_cast({online, UID, Pid, DID}, State) ->
    ...
    % 检查离线消息
    logic_msg_c2c:check_msg(UID, Pid, DID),
    % 检查群聊离线消息
    logic_msg_c2g:check_msg(UID, Pid, DID),
    {noreply, State, hibernate};
...
notice_friend(UID, State) ->
    Column = <<"`to_user_id`">>,
    case ds_friend:find_by_uid(UID, Column) of
        [] ->
            ok;
        Friends ->
            % ?LOG([State, Friends]),
...
send_state_msg(FromId, State, [[{<<"to_user_id">>, ToUID}] | Tail]) ->
    case repo_chat_store:lookup(ToUID) of
        [] ->
            ok;
        List ->
...
send_msg_code1019(From, To, ToPid, _State) ->
    ?LOG([From, To, ToPid]),
    % 用户在线状态变更
    Msg = ds_message:s2c(<<"1019">>, <<"">>, From, To),
