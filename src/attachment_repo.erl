...
-spec save(integer(), binary(), binary(), list()) ->
    {ok, list(), list()} | {error, any()}.
save(_Conn, _CreatedAt, _Uid, []) ->
    ok;
save(Conn, CreatedAt, Uid, [Attach|Tail]) ->
        Column1 = <<"(md5,mime_type,ext,name,path,url,size,info,referer_time,last_referer_user_id,last_referer_at,creator_user_id,updated_at,created_at,status)">>,
