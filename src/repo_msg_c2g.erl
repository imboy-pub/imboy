-module(repo_msg_c2g).
%%%
% repo_msg_c2g 是 msg_c2g repository 缩写
%%%
-export([write_msg/6]).
...
write_msg(CreatedAt, Id, Payload, FromId, ToUids, Gid) ->
    ...
    poolboy:transaction(mysql, fun(ConnectPid) ->
        ...
        Vals = lists:map(fun(ToId) ->
            ...
            case repo_msg_c2g_timeline:count_by_to_id(ToId) of
               {ok, _,
                    [[Count]]}
                 when Count >=
                      ?SAVE_MSG_LIMIT ->
                   Limit =
                       ...
                       ?SAVE_MSG_LIMIT +
                       1,
                   case repo_msg_c2g_timeline:delete_overflow_timeline(ToId,
                                                                       Limit) of
                       {msg_ids, MsgIds} ->
                           [case repo_msg_c2g_timeline:check_msg(CheckMsgId) of
                                {ok, _,
                                     [[0]]} ->
                                    ds_msg_c2g:delete_msg(CheckMsgId);
                                {ok, _,
                                     _} ->
                                    ok
                            end ||
