...
do_list(<<"GET">>, Req0, State) ->
    case ensure_permission(State, <<"plugins:read">>, Req0) of
        ok ->
            ...
            case get_name(Req0) of
                ...
                Name ->
                    case imboy_plugin_manager:get_plugin(Name) of
                        {ok, Info} -> elib_response:success(Req0, Info);
                        {error, not_found} -> elib_response:error(Req0, <<"插件不存在"/utf8>>, ?ERR_NOT_FOUND)
                    end
            end;
        {error, Req1} ->
            Req1
            ...
            case get_name(Req0) of
                ...
                Name ->
                    case imboy_plugin_manager:get_state(Name) of
                        {ok, Info} -> elib_response:success(Req0, Info);
                        {error, not_found} -> elib_response:error(Req0, <<"插件不存在"/utf8>>, ?ERR_NOT_FOUND);
                        {error, _} -> elib_response:error(Req0, <<"进程错误"/utf8>>, ?ERR_BAD_REQUEST)
                    end
            end;
        {error, Req1} ->
            Req1
            ...
            case get_name(Req0) of
                ...
                Name ->
                    case imboy_plugin_manager:health_check(Name) of
                        {ok, Info} -> elib_response:success(Req0, Info);
                        {error, not_found} -> elib_response:error(Req0, <<"插件不存在"/utf8>>, ?ERR_NOT_FOUND);
                        {error, _} -> elib_response:error(Req0, <<"健康检查失败"/utf8>>, ?ERR_BAD_REQUEST)
                    end
            end;
        {error, Req1} ->
            Req1
...
do_install(<<"POST">>, Req0, State) ->
    case ensure_permission(State, <<"plugins:install">>, Req0) of
        ok ->
            PostVals = elib_param:post(Req0),
            case {maps:get(<<"name">>, PostVals, undefined),
                  maps:get(<<"path">>, PostVals, undefined)} of
                {undefined, _} ->
                    elib_response:error(Req0, <<"缺少插件名称"/utf8>>, ?ERR_BAD_REQUEST);
                {_, undefined} ->
...
do_enable(<<"POST">>, Req0, State) ->
    case ensure_permission(State, <<"plugins:enable">>, Req0) of
        ok ->
            case get_name(Req0) of
                undefined ->
                    elib_response:error(Req0, <<"缺少插件名称"/utf8>>, ?ERR_BAD_REQUEST);
                ...
                Name ->
                    case imboy_plugin_manager:enable(Name) of
                        ok -> elib_response:success(Req0, #{name => Name, state => enabled});
                        {error, not_found} ->
...
do_disable(<<"POST">>, Req0, State) ->
    case ensure_permission(State, <<"plugins:disable">>, Req0) of
        ok ->
            case get_name(Req0) of
                undefined ->
                    elib_response:error(Req0, <<"缺少插件名称"/utf8>>, ?ERR_BAD_REQUEST);
                ...
                Name ->
                    case imboy_plugin_manager:disable(Name) of
                        ok -> elib_response:success(Req0, #{name => Name, state => disabled});
                        {error, not_found} ->
                            elib_response:error(Req0, <<"插件不存在"/utf8>>, ?ERR_NOT_FOUND);
                        {error, Reason} ->
                            format_lifecycle_error(Req0, Reason)
...
do_upgrade(<<"POST">>, Req0, State) ->
    case ensure_permission(State, <<"plugins:upgrade">>, Req0) of
        ok ->
            PostVals = elib_param:post(Req0),
            case {maps:get(<<"name">>, PostVals, undefined),
                  maps:get(<<"version">>, PostVals, undefined)} of
                {undefined, _} ->
                    elib_response:error(Req0, <<"缺少插件名称"/utf8>>, ?ERR_BAD_REQUEST);
                {_, undefined} ->
                    ...
                    case safe_plugin_name(NameBin) of
                        ...
                        {ok, Name} ->
                            case imboy_plugin_manager:upgrade(Name, Version) of
                                ok -> elib_response:success(Req0, #{name => Name, version => Version});
                                {error, not_found} ->
                                    elib_response:error(Req0, <<"插件不存在"/utf8>>, ?ERR_NOT_FOUND);
                                {error, Reason} ->
                                    format_lifecycle_error(Req0, Reason)
...
do_uninstall(<<"POST">>, Req0, State) ->
    case ensure_permission(State, <<"plugins:uninstall">>, Req0) of
        ok ->
            PostVals = elib_param:post(Req0),
...
do_reset(<<"POST">>, Req0, State) ->
    case ensure_permission(State, <<"plugins:reset">>, Req0) of
        ok ->
            case get_name(Req0) of
                undefined ->
                    elib_response:error(Req0, <<"缺少插件名称"/utf8>>, ?ERR_BAD_REQUEST);
                ...
                Name ->
                    case imboy_plugin_manager:reset(Name) of
                        ok -> elib_response:success(Req0, #{name => Name, state => unknown});
                        {error, not_found} ->
                            elib_response:error(Req0, <<"插件不存在"/utf8>>, ?ERR_NOT_FOUND);
                        {error, Reason} ->
                            format_lifecycle_error(Req0, Reason)
...
do_force_uninstall(<<"POST">>, Req0, State) ->
    case ensure_permission(State, <<"plugins:force_uninstall">>, Req0) of
        ok ->
            PostVals = elib_param:post(Req0),
            NameBin = maps:get(<<"name">>, PostVals, undefined),
            Mode = maps:get(<<"mode">>, PostVals, <<"preserve_data">>),
...
safe_plugin_name(NameBin) when is_binary(NameBin), byte_size(NameBin) > 0, byte_size(NameBin) =< 64 ->
...
format_lifecycle_error(Req0, {deps_not_enabled, Names}) ->
    ...
    elib_response:error(Req0, <<"依赖未启用:"/utf8, (iolist_to_binary(
        lists:join(<<",">>, NameBins)))/binary>>, ?ERR_BAD_REQUEST);
format_lifecycle_error(Req0, {has_dependents, Names}) ->
    NameBins = [atom_to_binary(N) || N <- Names],
    elib_response:error(Req0, <<"存在依赖项:"/utf8, (iolist_to_binary(
        lists:join(<<",">>, NameBins)))/binary>>, ?ERR_BAD_REQUEST);
...
resolve_permissions_by_adm_user_id(AdmUserId) ->
    Key = {adm_user_plugin_permission, AdmUserId},
