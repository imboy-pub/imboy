...
%% @param OrderBy 排序字段
%% @return {ok, Columns, Rows} 查询成功返回列和行数据 | {error, Reason} 查询失败
-spec select_by_where(binary(), binary(), integer(), integer(), binary()) -> {ok, list(), list()} | {error, any()}.
select_by_where(Column, Where, Limit, Offset, OrderBy) ->
    ...
    Sql = <<"SELECT ", Column/binary, " FROM ", Tb/binary,
            " WHERE ", Where/binary, " order by ", OrderBy/binary,
            " LIMIT ", Limit2/binary, " OFFSET ", Offset2/binary>>,
    imboy_log:info(io_lib:format("adm_user_repo/select_by_where/5: Sql ~p ~n", [Sql])),
    imboy_db:query(Sql, []).
