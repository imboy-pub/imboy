...
process_offline_ack(Uid, <<"c2c">>, MsgIds) ->
    {ok, C} = delete_c2c_messages_by_ids(Uid, MsgIds),
    C;
process_offline_ack(Uid, <<"c2g">>, MsgIds) ->
    {ok, C} = delete_c2g_messages_by_ids(Uid,MsgIds),
    C;
process_offline_ack(Uid, <<"s2c">>, MsgIds) ->
    {ok, C} = delete_s2c_messages_by_ids(Uid, MsgIds),
    C;
process_offline_ack(_, _, _) ->
    {error, <<"internal_server_error">>}.

% 根据消息ID列表删除C2C消息
delete_c2c_messages_by_ids(Uid, MsgIds) ->
    % 使用单个 SQL 批量删除替代循环删除
...
% 根据消息ID列表删除C2G消息
delete_c2g_messages_by_ids(Uid, MsgIds) ->
    % 使用单个 SQL 批量删除替代循环删除
...
% 根据消息ID列表删除S2C消息
delete_s2c_messages_by_ids(Uid, MsgIds) ->
    % 使用单个 SQL 批量删除替代循环删除
