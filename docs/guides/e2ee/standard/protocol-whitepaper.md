...
### 3.1 ADR 状态头已经失真（本次核对新发现）

| ADR | 状态头写的 | 实际 |
|---|---|---|
| **27**（附件加密） | 「设计草案（**不实施**）……**本文件不改动任何生产代码**」 | **已实施**：分块 AEAD、AAD 绑定、加密闸门、临时明文清理、缩略图独立 key 全部落地，`imboyapp/test/service/e2ee/` 下 13 个附件测试文件。仅**运行时开关未翻开** |
| **28**（KT 调研） | 「调研与设计草案（**不改任何生产代码**）」 | Merkle 库已完成（未接线，gap-matrix B3） |

照状态头读，审计方会得出「附件加密根本没做」的结论——**与事实相反**。
真实状态：代码在、测试在、开关没翻。这两个状态头需修订，已登记。
