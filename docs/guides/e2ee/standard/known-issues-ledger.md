...
| IMB-2026-006 | **Safety Number 已接线（2026-08-25 更新）**。C2C 单聊设置页、联系人资料页、群成员详情页均提供"安全码验证"入口，展示 60 位 Signal FingerprintProtocol v2 兼容安全码（本端 identity 走 OlmSessionService 权威副本；对端 identity 走 Ed25519 自签核验 + TOFU pin 的已验证路径）。比对一致后调 `TrustRecordService.recordVerified` 先上报 `POST /api/v1/e2ee/trust/record`（method=manual_number），成功才本地标记已验证。 | `Resolved` | 原遗留①②③已闭环：① trust/record 上报已接线（`TrustRecordService` + `OlmApi.recordTrust`/`listDevices`，后端 `list_devices_with_identity` 返回 `device_generation`/`identity_version`，提交 `32080355`）；② 群聊成员级安全码入口已加（`group_member_detail_page.dart`，提交 `948df18d`）；③ 多设备聚合（Signal v2 语义）仍以双方主设备为准——本次未做多设备聚合，属后续增强。真机黑盒验收通过（`CLEAN_VERIFY_2026` 双端加解密 + TOFU 拒绝验证）。威胁模型 T2/T8 的**首触 MITM 用户可检测**已恢复 |
