...
| TT | 现状 | 证据 | 关闭任务 |
|---|---|---|---|
| D1 | ✅ **白皮书已建**（2026-08-02） | `standard/protocol-whitepaper.md`。**刻意不复制任何 wire 格式**——规范字节留在 ADR 作唯一真源；复制副本正是今日发现 `e2ee-key-rotation-policy.md` 失真的成因，再抄一遍只是预约下一次同样事故。改为提供 ADR 各自做不到的三件事：①**supersedes 生效解析表**（此前不存在；ADR 10 是依赖图、11 是版本兼容，都不解析"哪份现在生效"）——直接读 ADR 03/04/05/06/07/08/13 会读到已被 14/15/16/17/18/26 取代的条款，且 **ADR 19 仍 `Proposed` 故 MLS 的生效决定仍是 ADR 09「不实现」**；②按协议关注点的分段导航（16 段）；③规范与实现落差。**新发现两处 ADR 状态头失真**：ADR 27 写「设计草案（不实施）·不改动任何生产代码」而附件加密已实施（13 个测试文件，仅开关未翻）、ADR 28 写「不改任何生产代码」而 Merkle 库已完成——照状态头读会得出"附件加密根本没做"的反事实结论 | P5-3 |
...
| D5 | 🟡 **六件套已到 3/6**（2026-08-02） | ①威胁模型 ✅（D2）②密码学清单 ✅ **新建** `standard/crypto-inventory.md` + D3 许可证清单 ④密钥生命周期 ✅（D4）；③白皮书 / ⑤已知问题台账 / ⑥SOW / 附 A 自审计 / 附 B Evidence Manifest 仍缺。密码学清单每行标注 📄本仓实证 / 📕上游规范 / ⚙️上游默认三级证据等级，**新发现**：SQLCipher 分组密码/HMAC/KDF 全用库默认值、我方未显式选定也未做安全评估（三个 PRAGMA 关键字在 `lib/` 全部零命中）；另订正一处易混口径——消息通道是 **CBC+HMAC（Encrypt-then-MAC）非 GCM**，仅附件与备份用 GCM。未含 SBOM hash 锚 | P5-3/5-5/5-6/5-7/5-8 |
