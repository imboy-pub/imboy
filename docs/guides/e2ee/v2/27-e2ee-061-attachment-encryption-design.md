...
> **状态**（2026-08-02 更正）：**设计已实施，运行时开关未翻开**。
>
> 原状态头写「设计草案（**不实施**）……本文件不改动任何生产代码」——**该表述已失真**：
> Slice 1–8 已落地并接入生产（`attachment_encryptor` / `attachment_seal_policy` /
> `attachment_chunk_codec` / `attachment_binding` / `attachment_temp_hygiene` 各有
> 2–5 个 `lib/` 生产文件引用，`test/service/e2ee/` 下 13 个附件测试文件）。
> 仅 **Slice 9（运行时开关）** 未完成，真机验证 BLOCKED + 两项待拍板。
>
> ⚠️ **后果**：附件今天仍以**明文**存于对象存储，ATT-01..05 全部不成立。
> 判据是运行时开关，不是代码存在性。见 `standard/known-issues-ledger.md` IMB-2026-005、
> 威胁模型 T10。原表述会让读者得出「附件加密根本没做」的**反事实**结论。
