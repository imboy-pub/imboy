...
> 最后更新：2026-06-30
> 维护人：运维安全团队
> 关联文档：`docs/operations/security.md`、`docs/security/security-hardening.md`、`config/sys.config.example`
...
| 1.1.3 关键通信链路冗余 | 数据库/存储具备冗余路径 | ⚠️ 部分实现 | PostgreSQL 主从复制文档见 `docs/operations/postgres-read-replica.md`；生产是否已部署取决于客户环境，需客户自行确认 |
...
| 2.1.3 边界处对网络流量做访问控制 | 防火墙规则管理 | ✅ 已实现 | `deploy/` 内 `preflight.sh` 包含 ufw 规则检查；生产防火墙配置见 `docs/operations/deployment/DAY1-QUICKSTART.md` |
...
| 3.3.3 审计记录不可被普通用户删除 | 日志保护 | ⚠️ 部分实现 | lager 日志写本地文件，需 OS 层面配置 append-only 或写入 Loki（`docs/operations/deployment/MONITORING.md`）；Loki 日志不可删是架构设计但未强制配置 |
| 3.3.4 审计记录保存不少于 6 个月 | 日志保留期 | ⚠️ 部分实现 | Loki 保留期取决于磁盘配置；生产部署须在 `deploy/loki/loki-config.yml` 中设置 `retention_period: 180d`（当前未明确设置） |
| 3.3.5 [三级] 集中审计管理平台 | 日志统一收集、告警 | ⚠️ 部分实现 | Prometheus + Grafana + Loki 栈已就绪（`docs/operations/deployment/MONITORING.md`）；告警规则需客户按场景补充 |
...
| 3.5.3 备份数据完整性校验 | 备份验证 | ⚠️ 部分实现 | `imboy/scripts/backup_pg.sh` 实现备份；恢复演练记录见 `docs/operations/deployment/RESTORE-DRILL-2026-06.md`；未实现自动完整性校验哈希 |
...
| 4.1.3 集中监控管理 | Prometheus + Grafana | ✅ 已实现 | 监控栈见 `docs/operations/deployment/MONITORING.md`；metrics 端点 `/metrics` 仅内网可达（nginx 配置拦截外部访问） |
...
| 控制点 | 要求 | 状态 | 说明 |
|--------|------|------|------|
| 5.1 信息安全方针政策 | 明文安全策略文档 | ⚠️ 部分实现 | `docs/operations/security.md` 覆盖技术安全基线；企业级安全方针文件（PDF/OA 系统）需客户自行建立 |
...
| 6.3 供应链安全管理 | 第三方组件安全评估 | ⚠️ 部分实现 | `docs/operations/dependencies.md` 列出核心依赖；未实现 SCA（软件成分分析）自动扫描；建议集成 Snyk 或 OWASP Dependency Check |
...
| 证据类型 | 文件/位置 |
|----------|-----------|
| 网络拓扑图 | `docs/operations/deployment/production-architecture.md` |
...
| 备份恢复记录 | `docs/operations/deployment/RESTORE-DRILL-2026-06.md` |
| 漏洞扫描报告 | 测评前由第三方出具 |
| 安全培训记录 | 客户提供 |
| 安全策略文件 | 客户提供 |
