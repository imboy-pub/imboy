...
| E2EE | Strong base, contract closed | required mode gates, per-device envelopes；E-01（b1ea97f2）e2ee-visibility-matrix.md 双矩阵 + 契约套件 6/6（required 下明文不达 logs/admin/push：日志哨兵测试/admin audit 三态/push 静态占位）+ PFv3 信封真实判定防误伤；E-02 Android 侧已执行（2026-09-08，见 E-02 Checklist §2.1）：真机（MRD-AL00 ARM32）integration_test 4 文件全 PASS（C2C/C2G 出站帧 + Olm 5/5 + Megolm 1/1）+ 双账号服务端链 11/11（含 offline/明文拒收/泄漏扫描），DB 实证落库=Olm PFv3 信封 | E-02 iOS 全列/推送通知预览/key recovery/真机附件与拉黑走查 **BLOCKED**（缺设备/缺 provider/EMUI 注入限制），按计划准则不 PASS；admin UI 复核 PENDING（本地无 adm listener，服务端契约已有测试钉死） | P1 architecture conflict → 已关闭，剩验收（Android 侧已补） | Yes | 可见性矩阵即对外声明底稿；合规私钥托管披露见 e2ee-policy.md §3 |
