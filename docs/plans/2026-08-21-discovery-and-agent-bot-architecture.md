...
## 后续事项（2026-08-22 审计与修复后遗留）

### 迁移 00000071 魔法 ID 的碰撞地平线（M2，低优先级）

默认 Agent 的固定 ID `1000000000000000001/2` 位于 TSID 值域内。elib_tsid 布局为
`elapsed_ms(42bit) << 21 | node | seq`，纪元 2025-01-01：

- 2026 年当前 user TSID ≈ 1.1×10¹⁷，魔法 ID 1×10¹⁸ 有约 9 倍余量
- TSID 增长到 1×10¹⁸ 需 `1e18 >> 21` ≈ 15.1 年 → **约 2040 年 2 月起，
  真实注册用户的 TSID 将进入该区间，user.id 主键存在碰撞可能**

处置建议：2040 年前任意时点执行一次幂等重播迁移（删旧 ID 行 → 用 TSID 重建默认
Agent），或届时将该迁移标记为 obsolete。长期正解是改为运行时按 `llm_providers`
配置种子化（仅 seed 已配置的 provider，同时消除本迁移与部署配置耦合的问题）。

### Bot OAuth Grant（表已建，流程未实现）

`bot_oauth_grant` 表已随迁移 00000070 创建，但无任何 CRUD/授权流程代码，也无
消费方。当前 `send_message` 走 `api_token`（Bot 自身身份），不需要 OAuth。待出现
「Bot 代表用户操作」的真实场景再实现，遵循 YAGNI。

### 前端任务（本计划纯后端）

- imboyapp：群/频道发现页 UI、Agent 市场入口、Bot 聊天入口
- imboyadmin：Bot 管理页（对接 `/api/adm/bot/*` 四端点）
