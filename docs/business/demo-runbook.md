...
| 「怎么证明你们看不到？」 | CI 脚本 `check_server_zero_crypto.sh` grep 全部后端代码，发现解密函数调用即构建失败。开源可审计。**若客户已启用 compliance 模式，必须同时说明第二收件人机制（见下一行），不得只答这一句。** |
| 「有没有后门 / 我们能不能审计员工聊天？」 | **必须主动披露**：`compliance` 模式下客户端会额外用一枚合规公钥再 wrap 一份会话密钥，持合规私钥者可解密该模式全部消息。默认 `disabled`，需部署方显式开启；合规私钥由客户自己在 HSM/离线介质生成，**我方服务端永不接收、永不落盘**。不需要就配 `required`（纯端到端）。详见安全简报「合规密钥托管：必读披露」。 |
