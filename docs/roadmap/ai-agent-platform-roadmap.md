...
> **版本**：1.4.0 | **创建**：2026-07-09 | **最后更新**：2026-08-08 | **状态**：Phase 0/1/2/3 ✅ 已交付；新增 AI Agent 角色模板、版本发布、后台继承配置与运行时策略门控；Phase 4 未开工（前瞻大阶段，待立项）。⚠️ Phase 2 流式真机 E2E 未验（生产 `.env.pro` 无 Phase 2 代码/provider/agent 账号，需本地环境验证）
...
### 7.1 角色模板与助手继承（MVP 已落地）

为避免把“AI Agent / AI Role / AI 助手”做成三个相互竞争的概念，当前产品统一采用：

- **AI Agent（助手）**：可登录、可被单聊或群聊触发的运行实例，保留 provider、model、owner 等实例属性。
- **AI Role（角色模板）**：可复用的配置模板，集中维护 system prompt、能力策略和知识库策略。
- **助手与角色**：Agent 通过 `role_id` 绑定 Role；运行时优先使用已发布角色版本，未绑定或角色不可用时保留旧字段兼容回退。

后台最小闭环：角色分页查询 → 新建/编辑草稿 → 显式发布 → 启用/停用；助手管理页只选择角色，不再允许自由编辑 system prompt 或 capabilities。角色策略使用结构化控件表达知识库模式/来源/上下文上限、群回复和主动触达，避免把高成本能力默认打开。

验证边界：后端 EUnit 覆盖版本、继承、发布、禁用、能力门控、知识库限额与兼容回退；管理后台覆盖 API、角色编辑/发布确认、助手绑定；Playwright 覆盖角色发布和绑定助手保存链路。
