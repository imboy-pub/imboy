...
- verify: redocly lint api/openapi.yaml；make compile && make eunit；bun test && bun run build；flutter analyze && flutter test；存在基线失败时必须完成同基线对比并证明零新增失败，不得将零回归表述为绝对全绿。
...
- verify: 所有 deps done；三仓本次变更范围检查通过且相对基线无新增失败；注册→License→OIDC→订阅→mock 支付→审计→导出→备份 smoke 通过；迁移 up/down 成对且版本号唯一；git diff --check。
- post_merge_evidence: 合并后复验修复了 `00000050_attachment_cipher` 与 `00000050_billing_owner_uid` 的迁移冲突，将附件迁移重编号为 `00000052_attachment_cipher`（提交 `84398520`）；`bash scripts/check_release_consistency.sh` 18/18 通过。另修复未登录写端点的 204 空响应：`auth_ds` 现在返回 HTTP 401 + code=401 错误信封；`make eunit t=auth_ds_tests` 13/13 通过，商业冒烟已改为强制断言 HTTP 401、认证错误码和错误消息，最终 **47/47 通过**。
- evidence: **本地 mock 商业冒烟通过**：新增 `scripts/smoke/commercial_smoke.sh`（八段链路 45 项断言）+ `scripts/smoke/commercial_rpc.escript`（token/license/user_count/seed_plan/refresh/fresh_check，全部调运行中节点的真实业务函数，不绕校验），提交 `97607f5f` 与 `9eb62052`。`bash scripts/smoke/commercial_smoke.sh` = **通过 45 / 失败 0（exit 0）**：注册走真实 getcode→验证码落库→signup（无旁路）；License quota 实测拦截（max_users=100 < current_users=440 → code 402「用户数已达授权上限…」）且 `public_info/0` 七字段齐备、license_text/signature/private_key/reason 均不外泄；OIDC 未配置时 fail-closed 返回 400（非 500）+ fake IdP 全链路 `make eunit t=auth_oidc_logic_tests` 21 tests passed；订阅段端到端验证本波次修复点（归属人可见详情且 owner_uid 匹配、**非归属人查询返回空对象**、非归属人续费 code=403「无权操作该订阅」）；mock 支付走 MOCK_ 通道且重复支付幂等拦截；导出返回 legal_hold.supported=false、六字段齐备、**敏感键零残留**；审计 user_log type=130 计数增长且 body 含 action=user_data_export；备份 `scripts/backup_pg.sh` 产出 1,025,003 字节可用 dump。**冒烟过程抓到两个真 bug 与一个环境缺口**：① `chk_user_log_type` 仍是建库时的 ARRAY[100,102,110,901,902,903]，type=130 插入被 23514 check_violation 拦下，而审计写入包在 try...catch 里失败只记 ERROR —— GDPR 导出**一条审计都没留**（本地库 type=130 行数实测为 0），已由迁移 `00000051_user_log_type_export`（含 down，刻意不删既有审计行）修复，修复后计数 1→2→3 可复验；② 未登录调用 `/api/v1/billing/renew` 返回 **204 空响应而非 401+错误信封**（根因 `src/ds/auth_ds.erl:198` `do_authorization(undefined,...) -> {stop, Req}` 未回包），属 W0-ARCH-01-P0（tag: security）范畴，本闸门不越界修，冒烟改为断言「业务逻辑未被执行」并显式打印该缺陷；③ 本地库虽记 schema_migrations=50 但 `billing_subscription.owner_uid` 实际不存在，已用迁移 50 的幂等 up.sql 补齐（冒烟前置已加该列存在性断言）。**合并后复验**：E2EE 迁移 `00000050_attachment_cipher` 与商业化 `00000050_billing_owner_uid` 发生版本冲突，保留已进入商业化序列的 `00000050_billing_owner_uid`，将附件迁移重编号为 `00000052_attachment_cipher`（提交 `84398520`）；`bash scripts/check_release_consistency.sh` = **18 项通过 / 0 项失败**，`git diff --check` 通过，当前 `priv/migrations` 的 up/down 成对且版本号唯一。**其余验收**：全量 `make eunit` = Passed 4611 / Failed 125（基线 dd021b61 = 4560/125，判定为零新增失败，不表述为绝对全绿）；`bunx @redocly/cli@2.41.1 lint api/openapi.yaml` = valid，0 errors / 25 预存 warnings；`bash scripts/test/check_release_consistency_test.sh` = 18/0；imboyadmin `bun run lint` 干净、`bun run build` ✓ 384ms；后端 `git diff --check` CLEAN；`git diff main...HEAD` 扫描无新增密钥/联系方式/生产数据。**外部阻塞**：真实支付、应用商店签名/上架、SAML/LDAP/SCIM、法律宣传语仍按外部/决策项处理，不自动调用或假设完成。**三仓说明**：imboyapp/imboyadmin 本次变更范围测试通过；其全量基线问题（Flutter 89 failed、Admin rbac.test 挂起）保持原状并已明确记录。
...
| C0 | 8 | 8 | 0 | 0 | 0 | GATE-C0 done（冒烟 47/47 通过，外部阻塞已逐条标注） |
