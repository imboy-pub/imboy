...
- 架构重构：从 Umbrella 多应用架构合并为单应用架构，简化部署和维护
- 4层代码分层：引入 Handler → Logic → DS → Repo 架构，提升代码组织和可维护性
    - Handler 层（src/api/, src/adm/）：HTTP 请求处理，33 个模块
...
- 统一使用 error_code.hrl 宏定义
- 数据库迁移文件统一到 priv/migrations/
...
* 完善apps目录下所有函数的spec注解和函数文档；

# 0.7.0
feat: - 消息编辑/撤销功能与PostgreSQL 18升级

*  新增消息编辑和撤销功能(单聊/群聊)
* 消息系统: C2C/C2G消息编辑、撤销功能完整实现
*  重构WebSocket消息处理逻辑，统一action处理机制，提升扩展性
*  升级到PostgreSQL 18，优化Docker配置
*  完善API文档和数据库迁移
*  优化时间戳处理和错误处理
* 文档: 新增聊天API文档，完善部署流程
...
* 新增 INFO_LOG/1 INFO_LOG/2 宏，用于在代码中打印信息日志；
