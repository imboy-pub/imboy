...
## 群组发现 / Group Discovery

> 公开群（type=1 且 status=1）检索；FTS 走 fts_group（jiebacfg，title A 权重 / introduction B 权重）。
> 前缀已废弃的搜索入口不存在；本组端点由 group_discovery_handler 提供。

| 方法 Method | 路径 Path | 鉴权 Auth | Handler#action | 用途 Purpose（中 / EN） | 请求参数 Request | 响应载荷 Response payload |
|---|---|---|---|---|---|---|
| GET | /api/v1/group/search | JWT | group_discovery_handler#search | 全文搜索公开群 / FTS search | `q`,`category_id`?,`page`,`size` | `{list,total}`（含 rank） |
| GET | /api/v1/group/discover | JWT | group_discovery_handler#discover | 发现页公开群 / Discover | `category_id`?,`sort`(popular/newest),`page`,`size` | `{list,total}` |
| GET | /api/v1/group/featured | JWT | group_discovery_handler#featured | 精选群 / Featured | `limit`(默认10) | `{list}` |
| GET | /api/v1/group/hot | JWT | group_discovery_handler#hot | 热门群（按成员数）/ Hot | `limit`(默认20) | `{list}` |
| GET | /api/v1/group/categories | JWT | group_discovery_handler#categories | 公开群分类 / Categories | - | `{list}` |
| GET | /api/v1/group/preview | JWT | group_discovery_handler#preview | 群预览（免加入）/ Preview | `group_id`(TSID) | 群公开字段 map |
...
| 方法 Method | 路径 Path | 鉴权 Auth | Handler#action | 用途 Purpose（中 / EN） | 请求参数 Request | 响应载荷 Response payload |
|---|---|---|---|---|---|---|
| GET | /api/v1/channels/search | JWT | channel_discovery_handler#search | 全文搜索频道（pg_jieba）/ FTS search | `q`,`category_id`?,`page`(默认1),`size`(默认20) | `{list,total}` |
| GET | /api/v1/channels/discover | JWT | channel_discovery_handler#discover | 发现页频道列表 / Discover | `category_id`?,`sort`(popular/newest/active),`page`,`size` | `{list,total}` |
| GET | /api/v1/channels/featured | JWT | channel_discovery_handler#featured | 精选频道 / Featured | `limit`(默认10) | `{list}` |
| GET | /api/v1/channels/trending | JWT | channel_discovery_handler#trending | 热门频道（统计加权）/ Trending | `period`(7d/30d),`limit`(默认20) | `{list}`（含 recent_* 统计） |
| GET | /api/v1/channels/categories | JWT | channel_discovery_handler#categories | 频道分类列表 / Categories | - | `{list}`（id,name,icon,sort_order） |
...
> Agent（account_type=1）是平台 AI，用户零代码；与开发者 Bot（account_type=3）分离。
> 私聊 Agent 走 C2C（msg_c2c_logic → ai_agent_reply，含限流闸门）。

| 方法 Method | 路径 Path | 鉴权 Auth | Handler#action | 用途 Purpose（中 / EN） | 请求参数 Request | 响应载荷 Response payload |
|---|---|---|---|---|---|---|
| GET | /api/v1/agent/list | JWT | ai_agent_handler#list | 助手列表 / List assistants | `keyword`?,`page`,`size` | 分页助手卡片 |
| GET | /api/v1/agent/discover | JWT | ai_agent_handler#discover | 发现页助手（list 语义别名）/ Discover | 同 list | 同 list |
| GET | /api/v1/agent/search | JWT | ai_agent_handler#search | 搜索助手 / Search | `q`,`page`,`size` | 分页 |
| GET | /api/v1/agent/categories | JWT | ai_agent_handler#categories | 助手分类 / Categories | - | `{categories:[...]}` |

## Bot（开发者服务，用户侧）

> Bot（account_type=3）注册/编辑为开发者客户端职责；此处为终端用户检索。
> `POST /api/v1/bot/send_message` 用 api_token（Bearer）认证，非用户 JWT，详见 bot-developer-guide。

| 方法 Method | 路径 Path | 鉴权 Auth | Handler#action | 用途 Purpose（中 / EN） | 请求参数 Request | 响应载荷 Response payload |
|---|---|---|---|---|---|---|
| GET | /api/v1/bot/search | JWT | bot_handler#search | 搜索公开 Bot / Search bots | `q`,`page`,`size` | `{total,page,size,list}` |
| POST | /api/v1/bot/send_message | **api_token** | bot_handler#send_message | Bot 发消息给用户 / Bot sends | body `to_uid`(TSID),`msg_type`,`payload` | `{msg_id}`；限流 + has_exchange 前置校验 |
