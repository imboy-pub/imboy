...
| Finding 011 备份边界回归 | `imboyapp@f7fe8dc4`、`imboyapp@2da2ea5e`、`imboyapp@986d1ef3`；备份/恢复、Megolm 段、服务端密文包、parser 与恢复写失败边界 57/57 PASS；新增恢复核心定向复跑 11/11 PASS；备份导入 widget 6/6 PASS；定向 analyze、diff check 与提交门禁通过 | C；备份只纳入 legacy RSA 当前私钥和 Megolm inbound，明确排除 Olm account/session 与 TOFU pin；Secure Storage 秘密清单不可读时导出 fail-closed，任一 Megolm session 写入失败时导入不再显示完整成功，成功文案只承诺备份内已成功写入的 session；widget 现注入本地 cloud probe，不再构造生产 URL、读取 token 或进入密钥清理链，但仍只构成 C 级 UI 回归；当前工作区另有需要账号登录、云备份写删和宿主 Keychain 清理的暂存集成测试，超出本轮授权且未执行，不得作为攻击复测；真实换机/重装恢复仍 BLOCKED |
