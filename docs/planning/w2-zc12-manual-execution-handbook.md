...
- 数据源：生产等价规模快照（**脱敏**，PII 严禁离开授权环境）；
  - **工具已就绪（2026-08-30）**：`scripts/sanitized_snapshot.sh`——列级 default-deny 脱敏（知名 PII 列假号/置空、jsonb/tsvector 占位、自由文本置空或 `md5(主键)` 保唯一、ID/数值/时间/系统枚举保留）+ 残留 PII 扫描 + 行数对账，产出 `sanitized.dump`/`manifest.txt`/`mobile_map.csv`（假↔真映射，**严禁外发**）/`class.tsv`（分类审计）。
