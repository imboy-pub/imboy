...
> **职责 / Role**: 生产环境部署，包含 Docker Compose、Helm Chart、反向代理、可观测性栈
...
├── caddy/
│   └── Caddyfile                # 反向代理 + 自动 TLS (Let's Encrypt)
├── prometheus/
│   ├── prometheus.yml           # 抓取配置（4 个 job）
│   └── rules/
...
| 文件 | 说明 |
|------|------|
| `docker-compose.prod.yml` | 生产环境 6 服务编排入口，禁止直接修改数据库密码（改 .env） |
| `.env.example` | 所有必填环境变量的模板，**不要提交含真实密钥的 .env** |
| `preflight.sh` | 部署前自动检查依赖、端口占用、域名解析 |
| `caddy/Caddyfile` | 反向代理规则，后端 → `api.*`，管理后台 → `admin.*` |
...
- 生产环境必须修改 `.env` 中的 `JWT_SECRET`、`DB_PASSWORD`、`ADMIN_SECRET`，不得使用默认值。
