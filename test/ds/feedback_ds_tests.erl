...
add_creates_feedback_test_() ->
    ?WITH_MECKS(
        [
            ...
            {feedback_repo, [
                {'add', 11, fun(
                    Uid, Did, COS, COSV, AppVsn, Type, Rating, Contact, Body, Attach, Md5
                ) ->
                    ?assertEqual(1, Uid),
...
add_with_minimal_params_test_() ->
    ?WITH_MECKS(
        [
            ...
            {feedback_repo, [
                {'add', 11, fun(
                    _Uid, _Did, _COS, _COSV, _AppVsn, _Type, _Rating, _Contact, _Body, _Attach, _Md5
                ) ->
                    {ok, #{<<"id">> => 1}}
                end}
            ]}
...
add_with_duplicate_detection_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            Result = feedback_ds:add(
                Uid, Did, COS, COSV, AppVsn, Type, Rating, ContactDetail, Body, Attach
            ),
            ?assertEqual(ok, Result)
        end
    ).
...
%% BUG#134 回归（生产实证 8/13 + 8/16 四次 500 空 body）：
%% feedback_repo:add 真实契约是成功返回 ok（feedback_repo_tests L169 锁定），
%% 此前 case 只匹配 {ok, _}，ok 命中 case_clause → cowboy 500 → INSERT 已提交
%% 但客户端「操作失败」。真实契约下 add 必须返回 ok 且不 crash。
add_repo_returns_ok_test_() ->
    ?WITH_MECKS(
        [
            {elib_hasher, [
                {'md5', 1, fun(_Input) -> <<"repo_ok_md5">> end}
            ]},
            {elib_pg, [
                {'pluck_value', 5, fun(_Table, _Field, _Where, _Opts, _Default) -> 0 end}
            ]},
            {feedback_repo, [
                {'add', 11, fun(
                    ...
                    % 与真实 repo 契约一致（?assertEqual(ok, feedback_repo:add(...))）
                    ok
                end}
            ]}
        ],
