...
delete_success_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            % 反证：update_tag_in_objects 的 SQL 必须是 regexp_replace + 标签边界模式，
            % 否则尾标签（"name,"）与中间标签删不掉（批次72 实证残留）；
            % 且替换串必须是捕获组反向引用 \1（保留前导逗号），否则删中间/尾标签
            % 会吞掉分隔逗号 → 拼接相邻标签（alpha.23 生产实证残留 "qa-del-1"）
            Calls = meck:history(elib_pg),
            Up = [
                C
             || C <- Calls,
                element(2, element(2, C)) == execute,
                length(element(3, element(2, C))) == 3,
                begin
                    [_Conn, Sql, Params] = element(3, element(2, C)),
                    case Sql of
                        <<"UPDATE ", _/binary>> ->
                            binary:match(Sql, <<"regexp_replace">>) =/= nomatch andalso
                                length(Params) == 3 andalso
                                hd(Params) == <<"(^|,)", "标签1"/utf8, ",">> andalso
                                lists:nth(2, Params) == <<"\\1">> andalso
                                lists:nth(3, Params) == hd(Params);
                        _ ->
                            false
                    end
                end
    ...
    ?WITH_MECKS(
        [
            ...
            {elib_pg, [
                ...
                % 事务内 change_scene_tag/merge_tag 走 3 参版本（传事务连接）
        ...
        fun() ->
            ...
            ?assertEqual(ok, Result),
            % 反证：事务内必须走 3 参（传连接）版本，否则 change_scene_tag
            % 读不到未提交改名。query/3 与 execute/3 各应被调用 1 次。
            Calls = meck:history(elib_pg),
            % meck history: {Pid, {Mod, Fun, Args}, Ret}，Args 长度 3 = 3 参调用
            Q3 = [
                C
             || C <- Calls,
                element(2, element(2, C)) == query,
                length(element(3, element(2, C))) == 3
            ],
            E3 = [
                C
...
%% @doc 测试修改标签名称无关联对象（空标签也必须更新 user_tag 表，
%% 旧实现零写入假成功 —— 真机批次72 实证，此处回归）
change_name_without_relation_objects_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Result = user_tag_ds:change_name(0, 100, 1, 1, <<"新标签名"/utf8>>),
            ?assertEqual(ok, Result),
            % 空标签必须真的更新 user_tag（update_tag 被调用）
            Calls = meck:history(user_tag_relation_repo),
            UpdateCalls = [C || C <- Calls, element(2, element(2, C)) == update_tag],
            ?assert(length(UpdateCalls) == 1)
        end
    ).
