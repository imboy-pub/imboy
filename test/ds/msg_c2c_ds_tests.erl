...
-define(ELIB_DT_MOCK,
    {elib_dt, [
        {'to_rfc3339', 1, fun(Val) when is_integer(Val) ->
...
write_msg_v2_creates_message_test_() ->
    ...
    ?WITH_MECKS([
        ...
        Result = msg_c2c_ds:write_msg(CreatedAt, MsgId, Body, FromUid, ToUid, CreatedAt, MsgType, E2EE),
        ?assertEqual(ok, Result)
    end).
...
read_msg_returns_list_test_() ->
    ...
    ?WITH_MECKS([
        ...
        {elib_response, [
            {'json_decode_field', 2, fun(R, <<"payload">>) ->
...
find_msg_by_id_existing_msg_test_() ->
    MsgId = <<"msg_123">>,
    Msg = #{<<"from_id">> => 1, <<"to_id">> => 2, <<"created_at">> => <<"2026-01-01">>, <<"payload">> => <<"{}">>},
    ?WITH_MECK(msg_c2c_repo, [
