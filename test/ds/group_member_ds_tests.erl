...
list_member_all_success_test_() ->
    ?WITH_MECKS([
        ...
        ?assertEqual({ok, [#{<<"nickname">> => <<"user-1">>, <<"user_id">> => 100}]},
...
join_group_success_test_() ->
    ?WITH_MECKS([
        ...
        ?assertEqual({ok, 100}, group_member_ds:join_group(self(), <<"invite">>, 100, 1, #{role => 1}))
    end).
...
alias_success_test_() ->
    ?WITH_MECKS([
        ...
        {elib_pg, [
            {'update', 4, fun(<<"group_member">>, Data, <<"group_id = $1 AND user_id = $2">>, [1, 100]) ->
