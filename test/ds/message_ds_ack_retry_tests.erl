    ...
    ?WITH_MECKS(log_mocks() ++ [
        {imboy_syn, [
            {'list_by_uid', 1, fun(1) ->
                ...
                [{PidAck, {<<"ios">>, <<"did_ack">>}}, {PidUnacked, {<<"ios">>, <<"did_unacked">>}}]
            end},
    ...
    ?WITH_MECKS(log_mocks() ++ [
        {imboy_syn, [
            {'list_by_uid', 1, fun(1) ->
                ...
                [{PidAck1, {<<"ios">>, <<"did_ack_1">>}}, {PidAck2, {<<"android">>, <<"did_ack_2">>}}]
            end},
            {'publish', 3, fun(_, _, _) -> erlang:error(unexpected_publish_call) end}
        ]},
        {imboy_cache, [
...
send_next_delayed_publish_replaces_old_timer_for_unacked_device_test_() ->
    ...
    ?WITH_MECKS(log_mocks() ++ [
        ...
        receive
            {delayed_pid_1, {timeout, _Ref, {[], {1, <<"did_1">>, <<"msg_3">>}, <<"raw_msg_3">>}}} ->
                ok
        after 400 ->
            ?assert(false)
        end,
...
send_next_race_ack_after_immediate_publish_only_retries_unacked_device_test_() ->
    ?WITH_MECKS(log_mocks() ++ [
        ...
        {imboy_cache, [
            {'get', 1, fun
                ({ack_received, 1, <<"did_ack_race">>, <<"msg_5">>} = Key) ->
                    Cnt = case get({ack_lookup_count, Key}) of undefined -> 0; N -> N end,
            ...
            {'set', 3, fun(Key, Ref, TTL) ->
                TimerSets = case get(test_timer_sets) of undefined -> []; V -> V end,
        ...
        receive
            {race_pending_pid, {timeout, _Ref3, {[], {1, <<"did_pending">>, <<"msg_5">>}, <<"raw_msg_5">>}}} ->
                ok
        after 500 ->
            ?assert(false)
        end,
        receive
            {race_ack_pid, {timeout, _Ref4, {[], {1, <<"did_ack_race">>, <<"msg_5">>}, <<"raw_msg_5">>}}} ->
                ?assert(false)
        after 250 ->
            ok
        end,
        ?assertEqual(2, get({ack_lookup_count, {ack_received, 1, <<"did_ack_race">>, <<"msg_5">>}})),
        ?assertEqual(1, meck:num_calls(imboy_cache, set, 3)),
