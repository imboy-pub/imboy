...
do_verify_sign_with_sha256_test_() ->
    ?WITH_MECK(elib_hasher, [
        ...
        ?assertEqual(true, auth_ds:do_verify_sign(<<"correct_sha256">>, <<"plaintext">>, <<"key">>, <<"sha256">>))
    end).

do_verify_sign_with_invalid_input_test() ->
    ?assertEqual(false, auth_ds:do_verify_sign(undefined, <<"plaintext">>, <<"key">>, <<"sha256">>)),
    ?assertEqual(false, auth_ds:do_verify_sign(<<"sign">>, <<"plaintext">>, undefined, <<"sha256">>)),
...
verify_token_with_valid_token_test_() ->
    ?WITH_MECK(token_ds, [
        {'decrypt_token', 1, fun(_Token) ->
            {ok, 123, <<"2026-03-16">>, <<"tk">>}
        end}
    ], fun() ->
        ?assertEqual({ok, 123}, auth_ds:verify_token(<<"Bearer valid_token">>))
    end).

verify_token_with_refresh_token_test_() ->
    ?WITH_MECK(token_ds, [
        {'decrypt_token', 1, fun(_Token) ->
            {ok, 123, <<"2026-03-16">>, <<"rtk">>}
        end}
    ], fun() ->
        ?assertEqual({error, ?ERR_TOKEN_REFRESH_NOT_ALLOWED, <<"TOKEN REFRESH NOT ALLOWED"/utf8>>},
