...
check_anomaly_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Result = e2ee_shard_transmission_log_repo:check_anomaly(
                ?TEST_KEY_VERSION, ?TEST_SHARD_ID
            ),
            ?assertMatch({ok, _}, Result)
        end
    ).

%% @doc 【E2EE-P2-15】保留期清理：批量 LIMIT + 天数参数
delete_older_than_builds_batched_delete_test_() ->
    ?WITH_MECKS(
        [
            {elib_pg, [
                {'execute', 2, fun(Sql, Params) ->
                    ?assertMatch(
                        {0, _}, binary:match(Sql, <<"DELETE FROM e2ee_shard_transmission_log">>)
                    ),
                    ?assertMatch({_, _}, binary:match(Sql, <<"make_interval(days => $1)">>)),
                    ?assertMatch({_, _}, binary:match(Sql, <<"LIMIT 1000">>)),
                    ?assertEqual([180], Params),
                    {ok, 42}
                end}
            ]}
        ],
        fun() ->
            ?assertEqual({ok, 42}, e2ee_shard_transmission_log_repo:delete_older_than(180))
        end
    ).
