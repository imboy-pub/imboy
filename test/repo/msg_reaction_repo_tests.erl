...
add_with_valid_data_returns_ok_test_() ->
    ?WITH_MECKS(
        [
            ...
            {elib_pg, [
                %% CI-00 修桩：add 实走 elib_pg:execute/2（repo 重构后 SQL 内联），
                %% 原 query/2 桩不命中 -> passthrough -> noproc。
...
add_with_duplicate_returns_ok_test_() ->
    ?WITH_MECKS(
        [
            ...
            {elib_pg, [
                %% CI-00 修桩：add 实走 elib_pg:execute/2（同上）
...
find_by_msg_emoji_filters_by_emoji_test_() ->
    ?WITH_MECK(
        ...
        fun() ->
            {ok, Reactions} = msg_reaction_repo:find_by_msg_emoji(
                <<"msg_005">>, <<"c2c">>, <<"👍"/utf8>>
            ),
...
add_with_database_error_returns_error_test_() ->
    ?WITH_MECKS(
        [
            ...
            {elib_pg, [
                %% CI-00 修桩：add 实走 elib_pg:execute/2（同上）
                {'execute', 2, fun(_Sql, _Params) -> {error, db_error} end}
            ]}
        ],
        fun() ->
