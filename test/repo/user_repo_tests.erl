...
find_by_id_existing_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            ?assertMatch(
                #{<<"id">> := FoundUid, <<"account">> := Account} when
                    is_integer(FoundUid) andalso is_binary(Account) andalso byte_size(Account) > 0,
                Result
            ),
...
%% 回归：真库集成测试实测复现 PG 42701 "column id specified more than once"。
%% normalize_legacy_create_data/1 用 atom `id` key 承载调用方传入的 id/uid，
%% 而 save/1 曾用 binary `<<"id">>` key 无条件覆盖——两个 key 类型不同、
%% Erlang map 不会互相覆盖，两者同时存在时 elib_pg_sql:insert/2 各自转换
%% 列名不去重，拼出的 INSERT 语句里 "id" 列出现两次。
save_with_atom_id_key_does_not_duplicate_id_column_test_() ->
    ?WITH_MECKS(
        [
            ?MOCK_ENV,
            ?MOCK_TSID,
        ...
        fun() ->
            %% 模拟 normalize_legacy_create_data/1 的产出：调用方原始 id/uid
            %% 落在 atom `id` key 上
            Data = #{
                id => 999,
                <<"account">> => <<"test_user_dup_id">>,
                <<"password">> => <<"hashed_password">>
            },
            Result = user_repo:save(Data),
...
save_with_both_id_and_binary_id_keys_keeps_single_column_test_() ->
    ?WITH_MECKS(
        [
            ?MOCK_ENV,
            ?MOCK_TSID,
...
%% @private 从 "INSERT INTO tb (col1,col2,...) VALUES (...)" 中取出列清单，
%% 统计某列名作为独立列出现的次数（按逗号切分后精确匹配，不会被
%% "account_id" 之类含 "id" 子串的列名误伤）。
count_occurrences(Sql, Col) ->
    [_, Rest] = binary:split(Sql, <<"(">>),
    [ColsPart, _] = binary:split(Rest, <<")">>),
    Cols = binary:split(ColsPart, <<",">>, [global]),
    length([C || C <- Cols, C =:= Col]).
...
select_by_where_basic_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            Result = elib_pg:page_with_total(
                user_repo:tablename(), Column, #{<<"__raw">> => Where}, OrderBy, Page, Limit
            ),
            case Result of
