...
upsert_test() ->
    ?WITH_MECKS([elib_pg, elib_dt, elib_pg_sql], fun() ->
        ...
        meck:expect(elib_pg, insert, fun(_Tb, _Data) -> {ok, 1} end),
        ?assertEqual({ok, 1}, push_token_repo:upsert(1, <<"did1">>, <<"android">>, <<"fcm">>, <<"token123">>)),
        %% 验证先 deactivate 再 insert
        ?assertEqual(2, meck:num_calls(elib_pg, execute, '_') + meck:num_calls(elib_pg, insert, '_'))
    end).
