    ...
    ?WITH_MECKS([
        {elib_pg, [
            {'query', 2, fun(Sql, [11, 50]) ->
                SqlBin = iolist_to_binary(Sql),
                ?assert(re:run(SqlBin, <<"ORDER BY is_pinned DESC, subscribed_at DESC LIMIT \\$2">>) =/= nomatch),
                {ok, [#{<<"user_id">> => 1001}]}
            end}
        ]}
    ], fun() ->
        ?assertMatch({ok, [#{<<"user_id">> := 1001}]}, channel_subscription_repo:list_by_channel(11, 0, 50))
    end).
...
delete_soft_sets_status_zero_test_() ->
    ?WITH_MECKS([
        {elib_pg, [
            {'update', 4, fun(<<"public.channel_subscription">>, #{status := 0}, <<"channel_id = $1 AND user_id = $2 AND status = 1">>, [11, 1001]) ->
                {ok, 1}
            end}
        ]}
    ], fun() ->
...
delete_with_conn_soft_sets_status_zero_test_() ->
    ?WITH_MECKS([
        {elib_pg, [
            {'update', 5, fun(fake_conn, <<"public.channel_subscription">>, #{status := 0}, <<"channel_id = $1 AND user_id = $2 AND status = 1">>, [11, 1001]) ->
                {ok, 1}
            end}
        ]}
    ], fun() ->
...
clear_unread_sets_zero_and_updates_last_read_at_test_() ->
    ?WITH_MECKS([
        ...
        {elib_pg, [
            {'update', 4, fun(<<"public.channel_subscription">>, #{unread_count := 0, last_read_at := 1700000000000}, <<"channel_id = $1 AND status = 1">>, [11]) ->
                {ok, 5}
            end}
        ]}
    ], fun() ->
...
clear_unread_with_uid_includes_user_in_where_clause_test_() ->
    ?WITH_MECKS([
        ...
        {elib_pg, [
            {'update', 4, fun(<<"public.channel_subscription">>, #{unread_count := 0, last_read_at := 1700000000000}, WhereClause, [11, 1001]) ->
