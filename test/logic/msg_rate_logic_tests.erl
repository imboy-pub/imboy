...
%% @doc 窗口重置：reset_counters 清零后，此前累积到警告档的用户重新放行
window_reset_test_() ->
    {setup, fun setup/0, fun cleanup/1, fun(_) ->
        Uid = 8001,
        %% 累积到超过警告阈值（31 条 → 第 31 条 warning）
        lists:foreach(
            fun(_) ->
                msg_rate_logic:check_and_record(Uid)
            end,
        ...
        ?assertMatch({warning, _}, msg_rate_logic:check_and_record(Uid)),
        %% 窗口重置（定时器每分钟做同样的事）
        ok = msg_rate_logic:reset_counters(),
        %% 计数归零 → 重新放行
        [?_assertEqual(ok, msg_rate_logic:check_and_record(Uid))]
    end}.
