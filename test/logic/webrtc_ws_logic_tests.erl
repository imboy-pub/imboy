...
event_sends_webrtc_server_ack_to_self_on_success_test_() ->
    %% B1 回执：成功路径必须向发送方 WS 进程（self()）投递
    %% {reply, JsonBin}，type=WEBRTC_SERVER_ACK 且 id 回显原 MsgId，
    %% 否则客户端机制A对每条 webrtc 信令报"消息确认超时"。
    ?WITH_MECKS(
        [
            {friend_ds, [
                {'is_friend', 2, fun(_ToUid, _CurrentUid) -> true end}
        ...
        fun() ->
            % Xid 字符串 id
            MsgId = <<"d94ie58821h5446u8ctg">>,
            ok = webrtc_ws_logic:event(123, 456, MsgId, <<"{\"type\":\"offer\"}">>),
            AckBin =
                receive
                    {reply, Bin} when is_binary(Bin) -> Bin
                after 100 ->
                    ?assert(false, "no {reply, AckBin} received")
                end,
            Ack = jsone:decode(AckBin, [{object_format, map}]),
            ?assertEqual(<<"WEBRTC_SERVER_ACK">>, maps:get(<<"type">>, Ack)),
            ?assertEqual(MsgId, maps:get(<<"id">>, Ack)),
            ?assertEqual(MsgId, maps:get(<<"in_reply_to">>, Ack)),
            ?assert(is_integer(maps:get(<<"server_ts">>, Ack)))
        end
    ).
...
event_forwards_ice_candidate_to_friend_test_() ->
    ?WITH_MECKS(
        [
            ...
            {message_ds, [
                {'send_next', 4, fun(_ToUid, _MsgId, Msg, _MsLi) ->
                    ...
                    ?assert(
                        string:find(Msg, "ice_candidate") =/= nomatch orelse
        ...
        fun() ->
            ...
            IceCandidate =
                <<"{\"type\":\"ice_candidate\",\"candidate\":\"candidate:1 1 UDP 2130706431 192.168.1.1 54321 typ host\"}">>,
