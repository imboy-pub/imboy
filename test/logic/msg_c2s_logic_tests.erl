    ...
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            Data = #{
                <<"to">> => <<"bot_qian_fan">>,
                <<"payload">> => #{
                    <<"text">> => <<"hello">>, <<"topic_id">> => 0, <<"topic_title">> => <<>>
                },
...
handle_sync_returns_messages_for_valid_c2c_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Result = msg_c2s_logic:handle_sync(
                100,
                [#{<<"conv_key">> => <<"c2c:100:200">>, <<"seq">> => 0}],
                50
            ),
...
c2s_to_external_with_stage_failure_fails_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            To = <<"bot_qian_fan">>,
            Payload = #{
                <<"text">> => <<"测试消息"/utf8>>, <<"topic_id">> => 0, <<"topic_title">> => <<>>
            },
            Data = #{
                <<"payload">> => Payload,
                <<"created_at">> => 1704067200000
...
c2s_to_external_without_topic_succeeds_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            To = <<"bot_qian_fan">>,
            Payload = #{
                <<"text">> => <<"简单问题"/utf8>>, <<"topic_id">> => 0, <<"topic_title">> => <<>>
            },
            Data = #{
                <<"payload">> => Payload,
                <<"created_at">> => 1704067200000
...
c2s_to_external_with_unicode_text_succeeds_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            To = <<"bot_qian_fan">>,
            Payload = #{
                <<"text">> => <<"你好，世界！🌍"/utf8>>, <<"topic_id">> => 0, <<"topic_title">> => <<>>
            },
            Data = #{
                <<"payload">> => Payload,
                <<"created_at">> => 1704067200000
...
c2s_to_external_with_large_topic_id_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            Payload = #{
                <<"text">> => <<"大主题 ID 消息"/utf8>>,
                <<"topic_id">> => LargeTopicId,
                <<"topic_title">> => <<"大主题"/utf8>>
            },
            Data = #{
                <<"payload">> => Payload,
                <<"created_at">> => 1704067200000
...
c2s_to_external_with_long_topic_title_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            To = <<"bot_qian_fan">>,
            % 构造超长标题
            LongTitle = binary:copy(<<"非常长的主题标题"/utf8>>, 100),
            Payload = #{
                <<"text">> => <<"消息"/utf8>>, <<"topic_id">> => 1, <<"topic_title">> => LongTitle
            },
            Data = #{
                <<"payload">> => Payload,
                <<"created_at">> => 1704067200000
...
c2s_to_external_with_utf8_text_succeeds_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            To = <<"bot_qian_fan">>,
            Payload = #{
                <<"text">> => <<"你好，这是一条中文消息"/utf8>>, <<"topic_id">> => 0, <<"topic_title">> => <<>>
            },
            Data = #{
                <<"payload">> => Payload,
                <<"created_at">> => 1704067200000
