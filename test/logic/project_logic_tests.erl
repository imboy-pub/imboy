    ...
    [
        {"guest cannot update (403)", fun() ->
            ?WITH_MECKS(base_mocks(), fun() ->
                ?assertMatch(
                    {error, {403, _}},
                    project_logic:update(?GUEST, ?PROJECT_ID, <<"新名">>, undefined)
                )
            end)
        end},
        {"member can update profile", fun() ->
            ?WITH_MECKS(base_mocks(), fun() ->
                ?assertMatch(
                    {ok, _}, project_logic:update(?MEMBER2, ?PROJECT_ID, <<"新名">>, <<"描述">>)
                ),
                receive
        ...
        {"archived workspace rejects update with 980", fun() ->
            ...
            ?WITH_MECKS(base_mocks(), fun() ->
                ?assertMatch(
                    {error, {980, _}},
                    project_logic:update(?MEMBER2, ?PROJECT_ID, <<"新名">>, undefined)
                )
            end)
        end}
    ].
