...
%% ===================================================================
%% 受限于 websocket_handler:init/2 依赖 cowboy_req 细节较多（已在 websocket_handler_tests 覆盖
%% v1/v2 协商的 State 装配），此处直接测 negotiate_protocol/1 的行为语义，
%% 这是握手完整链路里唯一决定 framing=v2 的分支点。
%% 注意：negotiate_protocol/1 为 handler 内部函数，不导出；此处通过 imboy_codec 的
%% framing_atom/1 + protocol_atom/1 组合验证（negotiate_protocol/1 就是这两者的转发）。

v2_subprotocol_maps_to_protobuf_v2_test_() ->
    ?TEST_SIMPLE(fun() ->
...
v2_negotiate_full_header_test_() ->
...
%% ===================================================================
v2_heartbeat_ping_pong_roundtrip_test_() ->
...
%% ===================================================================
v2_msg_c2c_json_routes_to_router_test_() ->
...
%% ===================================================================
%% 验证 dispatch_v2_frame(?FRAME_TYPE_MSG_C2S, <<"CLIENT_ACK,", ...>>, State)
%% 走 handle_client_ack 特判路径 —— 注意：该分支在 throttle 检查之前匹配，
%% 所以 throttle mock 抛 blocked 也必须放行。
v2_msg_c2s_client_ack_bypasses_throttle_test_() ->
    ?WITH_MECKS(log_mocks() ++ [
        {auth_ds, [
            {'current_uid', 1, fun(_State) -> 123 end}
        ]},
...
%% ===================================================================
v2_downstream_s2c_wraps_msg_s2c_frame_test_() ->
    ?WITH_MECKS(log_mocks(), fun() ->
        State = #{protocol => protobuf, framing => v2,
...
%% ===================================================================
v2_msg_c2c_garbage_payload_test_() ->
    ?WITH_MECKS(log_mocks() ++ [
        {auth_ds, [
            {'current_uid', 1, fun(_State) -> 123 end}
        ]},
...
%% ===================================================================
v2_bad_magic_frame_tolerated_test_() ->
    ?WITH_MECKS(log_mocks(), fun() ->
...
%% ===================================================================
v2_frame_ack_adapts_to_client_ack_test_() ->
    ?WITH_MECKS(log_mocks() ++ [
        {auth_ds, [
            {'current_uid', 1, fun(_State) -> 123 end}
        ]},
