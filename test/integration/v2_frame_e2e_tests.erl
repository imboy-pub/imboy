...
%%%   6. v2 垃圾 payload（随机 3 字节）不 crash，返回协议 ERROR 帧
%%%   7. v2 帧头部非法（非 "IB" magic）不 crash，返回协议 ERROR 帧
%%%
...
all_tests() ->
    [
        ...
        v2_frame_c2g_ack_routes_to_group_ack_pipeline()
    ].
...
%% ===================================================================
%% 6. v2 垃圾 payload（3 字节随机）返回协议 ERROR 帧且不 crash
%% ===================================================================
...
v2_msg_c2c_garbage_payload() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            {reply, {binary, ErrorBin}, State2, hibernate} =
                websocket_handler:websocket_handle({binary, Frame}, State),
            ?assertEqual(State, State2),
            assert_v2_error_frame(ErrorBin)
        end
    ).

%% ===================================================================
%% 7. v2 帧头部非法（bad magic）返回协议 ERROR 帧且不 crash
%% ===================================================================
...
assert_v2_error_frame(ErrorBin) ->
    {ok, ErrorFrame} = imboy_codec:unwrap_v2_frame(ErrorBin),
    ?assertEqual(?FRAME_TYPE_ERROR, imboy_frame:type(ErrorFrame)),
    ?assert(byte_size(imboy_frame:payload(ErrorFrame)) > 0).
...
%% v2 ACK 的方向位不能只在解析层验证；必须一路传到正确的业务 ACK 管道。
v2_frame_c2g_ack_routes_to_group_ack_pipeline() ->
    ?WITH_MECKS(
        log_mocks() ++
            [
                ...
                {websocket_logic, [
                    {'cancel_timer', 3, fun(
                        123, <<"did_frame_ack_c2g">>, <<"1311768467294899695">>
                    ) ->
                        ok
                    end}
                ]},
                {elib_dt, [
                    {'millisecond', 0, fun() -> 1700000000222 end}
                ]},
                {msg_c2g_logic, [
                    {'c2g_client_ack', 3, fun(
                        <<"1311768467294899695">>, 123, <<"did_frame_ack_c2g">>
                    ) ->
                        ok
                    end}
                ]}
        ...
        fun() ->
            State = #{
                did => <<"did_frame_ack_c2g">>,
                current_uid => 123,
                protocol => protobuf,
                framing => v2
            },
            MsgIdInt = 16#1234567890ABCDEF,
            AckFrame = imboy_frame:ack(MsgIdInt, c2g),
            Reply = websocket_handler:websocket_handle({binary, AckFrame}, State),
            ?assertMatch({reply, _, _, hibernate}, Reply),
            ?assertEqual(1, meck:num_calls(msg_c2g_logic, c2g_client_ack, 3))
        end
    ).
