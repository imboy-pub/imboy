...
init_list_success_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Req = #{},
            {ok, RespReq, _State} = adm_role_handler:init(Req, #{
            ...
            {elib_param, [
                {'post', 1, fun(_Req) ->
                    #{
                        <<"role_id">> => 2,
                        <<"permissions">> => [
                            <<"dashboard:view">>, <<"reports:read">>, <<"reports:read">>
                        ]
                    }
                end}
            ]},
...
init_permissions_save_forbidden_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Req = #{},
            {ok, RespReq, _State} = adm_role_handler:init(Req, #{
                action => permissions_save, adm_user_id => 3001
            }),
...
%% ===================================================================
%% 角色停用/删除（disable / delete action）
%% ===================================================================

init_disable_success_test_() ->
    ?WITH_MECKS(
        [
            {cowboy_req, [
                {'method', 1, fun(_Req) -> <<"POST">> end}
            ...
            {elib_pg, [
                ...
                {'update', 4, fun(<<"public.adm_role">>, Set, <<"id = $1">>, [9]) ->
                    ?assertEqual(0, maps:get(<<"status">>, Set)),
                    {ok, 1}
                end}
            ]},
            {elib_response, [
...
init_disable_builtin_role_rejected_test_() ->
    ?WITH_MECKS(
        [
            {cowboy_req, [
                {'method', 1, fun(_Req) -> <<"POST">> end}
            ...
            {elib_param, [
                {'post', 1, fun(_Req) -> #{<<"role_id">> => <<"1">>} end}
            ]},
            {elib_response, [
                {'error', 3, fun(Req, Msg, _Code) ->
        ...
        fun() ->
            ...
            ?assertEqual(<<"内置角色不可停用或删除"/utf8>>, maps:get(error_msg, RespReq))
        end
    ).

init_delete_blocked_when_admins_in_use_test_() ->
    ?WITH_MECKS(
        [
            {cowboy_req, [
                {'method', 1, fun(_Req) -> <<"POST">> end}
            ...
            {adm_user_ds, [
                {'count_by_role', 1, fun(9) -> {ok, 2} end}
            ]},
            {elib_response, [
                {'error', 3, fun(Req, Msg, _Code) ->
                    Req#{response_status => 400, error_msg => Msg}
        ...
        fun() ->
            {ok, RespReq, _State} = adm_role_handler:init(#{}, #{
                action => delete, adm_user_id => 1001
            }),
            ?assertEqual(400, maps:get(response_status, RespReq)),
            ?assertNotEqual(nomatch, binary:match(maps:get(error_msg, RespReq), <<"管理员在用"/utf8>>))
        end
    ).

init_delete_success_when_no_admins_test_() ->
    ?WITH_MECKS(
        [
            {cowboy_req, [
                {'method', 1, fun(_Req) -> <<"POST">> end}
            ...
            {adm_user_ds, [
                {'count_by_role', 1, fun(9) -> {ok, 0} end}
            ]},
            {elib_response, [
                {'success', 2, fun(Req, _Payload) -> Req#{response_status => 200} end}
            ]}
