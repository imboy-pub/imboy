...
captcha_generates_image_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            ?ASSERT_EQUAL(
                <<"image/png; charset=utf-8">>, maps:get(<<"content-type">>, Headers, undefined)
            ),
            ?assert(
                maps:is_key(<<"captcha_key">>, Req) orelse maps:get(has_captcha_cookie, Req, false)
            )
        end
    ).
...
login_page_returns_html_with_csrf_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            ?ASSERT_EQUAL(200, StatusCode),
            ?ASSERT_EQUAL(
                <<"text/html; charset=utf-8">>, maps:get(<<"content-type">>, Headers, undefined)
            ),
...
login_post_success_with_valid_credentials_test_() ->
    ?WITH_MECKS(
        [
            ...
            {elib_req, [
                {'cookie', 2, fun
                    ...
                    (<<"back_uri">>, _Req) -> <<"/api/adm/dashboard">>
                end}
            ]},
            {elib_param, [
                {'post', 1, fun(_Req) ->
        ...
        fun() ->
            ...
            ?ASSERT_EQUAL(<<"/api/adm/dashboard">>, maps:get(<<"next">>, ResponseData))
        end
    ).
