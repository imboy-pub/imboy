...
init_detail_success_with_owner_profile_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Req = #{method => <<"GET">>},
            {ok, RespReq, _State} = adm_group_handler:init(Req, #{
                action => detail, adm_user_id => 9001
            }),
            ?assertEqual(200, maps:get(response_status, RespReq)),
            Payload = maps:get(payload, RespReq),
...
init_dissolve_marks_group_disabled_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Req = #{method => <<"POST">>},
            {ok, RespReq, _State} = adm_group_handler:init(Req, #{
                action => dissolve, adm_user_id => 9001
            }),
            ?assertEqual(200, maps:get(response_status, RespReq)),
...
init_dissolve_permission_denied_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Req = #{method => <<"POST">>},
            {ok, RespReq, _State} = adm_group_handler:init(Req, #{
                action => dissolve, adm_user_id => 9003
            }),
            ?assertEqual(403, maps:get(response_status, RespReq)),
...
init_members_returns_paged_members_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Req = #{method => <<"GET">>},
            {ok, RespReq, _State} = adm_group_handler:init(Req, #{
                action => members, adm_user_id => 9001
            }),
            ?assertEqual(200, maps:get(response_status, RespReq)),
            Payload = maps:get(payload, RespReq),
            ?assertEqual(1, maps:get(total, Payload)),
...
init_search_requires_non_empty_keyword_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Req = #{method => <<"GET">>},
            {ok, RespReq, _State} = adm_group_handler:init(Req, #{
                action => search, adm_user_id => 9001
            }),
            ...
            [
                ...
                {group_task_repo, [
                    {'list_deleted_by_group_id', 3, fun(66, 1, 10) ->
                        {ok, [
                            #{<<"id">> => 9, <<"deleted_at">> => <<"2026-02-24T09:00:00+08:00">>}
                        ]}
                    end},
                ...
                {'query', 2, fun(Sql, Params) ->
                    ...
                    {ok, [
                        #{
                            ...
                            <<"nickname">> => <<"ops">>,
                            <<"body">> =>
                                <<"{\"action\":\"close_task\",\"operator_uid\":9002,\"group_id\":66,\"target_id\":99,\"occurred_at\":\"2026-02-24T08:00:00+08:00\",\"extra\":{\"scope\":\"task\"}}">>,
