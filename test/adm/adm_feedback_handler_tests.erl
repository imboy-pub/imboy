        ...
        [
            ...
            {elib_pg, [
                {'page_with_total', 6, fun(_Tb, _Column, _Where, _Order, 1, 20) ->
                    {ok, #{
                        list => [
                            #{<<"feedback_id">> => 11, <<"user_id">> => 9, <<"body">> => <<"ok">>}
                        ],
                        items => [legacy],
                        total => 1,
                        page => 1,
...
init_reply_converts_reply_pid_test_() ->
    ?WITH_MECKS(
        [
            ...
            {adm_user_logic, [
                {'find', 3, fun
                    (1, <<"id,role_id">>, _Key) -> #{<<"id">> => 1, <<"role_id">> => 1};
                    (1, <<"id,nickname">>, _Key) -> #{<<"nickname">> => <<"管理员"/utf8>>}
                end}
            ]},
            {adm_index_handler, [
...
%% 安全回归：无 feedback:reply 权限的管理员不能回复反馈（曾经零权限校验）
init_reply_permission_denied_test_() ->
    ?WITH_MECKS(
        [
            {cowboy_req, [
                {'method', 1, fun(_Req) -> <<"POST">> end}
        ...
        fun() ->
            {ok, Req, _State} = adm_feedback_handler:init(#{}, #{action => reply, adm_user_id => 9}),
    ...
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, Req, _State} = adm_feedback_handler:init(#{}, #{
                action => workflow_config, adm_user_id => 9
            }),
            ?assertEqual(?ERR_FORBIDDEN, maps:get(response_status, Req))
        end
    ).
...
workflow_config_get_returns_default_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            ...
            {ok, Req, _State} = adm_feedback_handler:init(#{}, #{
                action => workflow_config, adm_user_id => 1
            }),
            Payload = maps:get(payload, Req),
            ?assertEqual(200, maps:get(response_status, Req)),
...
workflow_config_put_normalizes_templates_and_sla_test_() ->
    ?WITH_MECKS(
        [
            ...
            {elib_param, [
                {'post', 1, fun(_Req) ->
                    #{
                        <<"reply_templates">> => [
                            <<" 模板A "/utf8>>, #{<<"text">> => <<"模板A"/utf8>>}, <<"模板B"/utf8>>
                        ],
                        <<"sla_hours">> => <<"9999">>
                    }
                end}
        ...
        fun() ->
            ...
            ?assertEqual(200, maps:get(response_status, Req)),
            ?assertEqual(
                [<<"模板A"/utf8>>, <<"模板B"/utf8>>], maps:get(<<"reply_templates">>, Payload)
            ),
