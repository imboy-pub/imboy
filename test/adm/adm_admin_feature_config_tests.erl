    ...
    ?WITH_MECKS(
        [
            ...
            {imboy_feature, [
                ...
                {'manifest_hash', 0, fun() -> <<"sha256:test">> end},
                {'manifest_schema_version', 0, fun() -> 1 end},
                {'compiled_features', 0, fun() -> [core, channel] end}
            ]},
            {elib_response, [
        ...
        fun() ->
            {ok, RespReq, State} = adm_admin_handler:init(#{}, #{
                action => config_features, adm_user_id => 1001
            }),
            ...
            ?assertEqual(
                #{
                    ...
                    <<"compiled_features">> => [<<"core">>, <<"channel">>]
                },
                maps:get(payload, RespReq)
            ),
            ?assertEqual(#{adm_user_id => 1001}, State)
        end
    ).

config_sidebar_includes_product_experience_test_() ->
    ?WITH_MECKS(
        [
            {cowboy_req, [
                {'method', 1, fun(_Req) -> <<"GET">> end}
        ...
        fun() ->
            {ok, RespReq, _State} =
                adm_admin_handler:init(#{}, #{action => config_sidebar, adm_user_id => 1001}),
            Items = maps:get(<<"items">>, maps:get(payload, RespReq)),
            Item = find_sidebar_item(<<"/settings/product-experience">>, Items),
            ?assertEqual([1], maps:get(<<"roles">>, Item)),
            ?assertEqual(<<"settings:view">>, maps:get(<<"permission">>, Item))
        end
    ).
...
init_config_policy_success_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, RespReq, State} = adm_admin_handler:init(#{}, #{
                action => config_policy, adm_user_id => 1001
            }),
            ?assertEqual(200, maps:get(response_status, RespReq)),
...
init_config_policy_bootstrap_success_test_() ->
    ...
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, RespReq, State} = adm_admin_handler:init(#{}, #{
                action => config_policy_bootstrap, adm_user_id => 1001
            }),
            ?assertEqual(200, maps:get(response_status, RespReq)),
...
init_config_policy_meta_success_test_() ->
    ...
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, RespReq, State} = adm_admin_handler:init(#{}, #{
                action => config_policy_meta, adm_user_id => 1001
            }),
            ?assertEqual(200, maps:get(response_status, RespReq)),
...
init_config_policy_saved_success_test_() ->
    ...
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, RespReq, State} = adm_admin_handler:init(#{}, #{
                action => config_policy_saved, adm_user_id => 1001
            }),
            ?assertEqual(200, maps:get(response_status, RespReq)),
...
preview_config_policy_post_success_test_() ->
    ...
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, RespReq, State} = adm_admin_handler:init(#{}, #{
                action => config_policy_preview, adm_user_id => 1001
            }),
            ?assertEqual(200, maps:get(response_status, RespReq)),
...
init_config_features_forbidden_without_settings_permission_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, RespReq, _State} = adm_admin_handler:init(#{}, #{
                action => config_features, adm_user_id => 3001
            }),
...
init_config_policy_forbidden_without_settings_permission_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, RespReq, _State} = adm_admin_handler:init(#{}, #{
                action => config_policy, adm_user_id => 3001
            }),
            ?assertEqual(403, maps:get(response_status, RespReq)),
            ?assertEqual(?ERR_FORBIDDEN, maps:get(error_code, RespReq)),
            ?assertEqual(<<"无权限操作"/utf8>>, maps:get(error_msg, RespReq)),
...
init_config_policy_bootstrap_forbidden_without_settings_permission_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, RespReq, _State} = adm_admin_handler:init(#{}, #{
                action => config_policy_bootstrap, adm_user_id => 3001
            }),
            ?assertEqual(403, maps:get(response_status, RespReq)),
            ?assertEqual(?ERR_FORBIDDEN, maps:get(error_code, RespReq)),
            ?assertEqual(<<"无权限操作"/utf8>>, maps:get(error_msg, RespReq)),
...
init_config_policy_meta_forbidden_without_settings_permission_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, RespReq, _State} = adm_admin_handler:init(#{}, #{
                action => config_policy_meta, adm_user_id => 3001
            }),
            ?assertEqual(403, maps:get(response_status, RespReq)),
            ?assertEqual(?ERR_FORBIDDEN, maps:get(error_code, RespReq)),
            ?assertEqual(<<"无权限操作"/utf8>>, maps:get(error_msg, RespReq)),
...
init_config_policy_saved_forbidden_without_settings_permission_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, RespReq, _State} = adm_admin_handler:init(#{}, #{
                action => config_policy_saved, adm_user_id => 3001
            }),
            ?assertEqual(403, maps:get(response_status, RespReq)),
            ?assertEqual(?ERR_FORBIDDEN, maps:get(error_code, RespReq)),
            ?assertEqual(<<"无权限操作"/utf8>>, maps:get(error_msg, RespReq)),
...
preview_config_policy_forbidden_without_settings_update_permission_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, RespReq, _State} = adm_admin_handler:init(#{}, #{
                action => config_policy_preview, adm_user_id => 3001
            }),
            ?assertEqual(403, maps:get(response_status, RespReq)),
            ?assertEqual(?ERR_FORBIDDEN, maps:get(error_code, RespReq)),
            ?assertEqual(<<"无权限操作"/utf8>>, maps:get(error_msg, RespReq)),
...
find_sidebar_item(_Path, []) ->
    #{};
find_sidebar_item(Path, [#{<<"path">> := Path} = Item | _]) ->
    Item;
find_sidebar_item(Path, [Item | Rest]) ->
    case find_sidebar_item(Path, maps:get(<<"children">>, Item, [])) of
        Found when map_size(Found) =:= 0 -> find_sidebar_item(Path, Rest);
        Found -> Found
    end.
