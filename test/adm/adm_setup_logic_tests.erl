...
is_initialized_db_error_fails_closed_test_() ->
    ?WITH_MECKS(
        [
            {adm_user_ds, [
                {'count', 0, fun() -> {error, unavailable} end}
            ]},
            {config_ds, [
                {'get', 1, fun(<<"adm.setup.completed_at">>) -> <<>> end}
            ]}
...
%% @doc 正常路径：空库 + 合法手机号 + 强密码 → 事务内创建首个管理员
do_init_happy_path_test_() ->
    ?WITH_MECKS(
        [
            {adm_user_ds, [
                {'count', 0, fun() -> {ok, 0} end},
                {'count', 1, fun(_Conn) -> {ok, 0} end},
                {'save', 2, fun(_Conn, _Row) -> {ok, 1} end}
            ]},
            {config_ds, [
                {'get', 1, fun(<<"adm.setup.completed_at">>) -> <<>> end}
            ]},
            {elib_pg, [
                ...
                {'execute', 3, fun(fake_conn, _Sql, []) -> {ok, 1} end}
            ]},
            {elib_type, [
