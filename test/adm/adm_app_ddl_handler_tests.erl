...
init_false_action_passthrough_test_() ->
    ?WITH_MECK(
        ...
        fun() ->
            Req = mock_request(),
            {ok, RespReq, State} = adm_app_ddl_handler:init(Req, #{
                action => false, adm_user_id => 100
            }),
        ...
        [
            ...
            {elib_pg, [
                {'page_with_total', 6, fun(Tb, Column, Where, OrderBy, Page, Size) ->
                    ...
                    ?assertEqual(
                        <<"id, ddl, down_ddl,old_vsn,new_vsn,status,updated_at,created_at">>, Column
                    ),
                    ?assertEqual(#{}, Where),
...
%% 安全回归：无 settings:ddl:delete 权限的管理员不能删除 DDL 配置（曾经零权限校验）
init_delete_permission_denied_test_() ->
    ?WITH_MECKS(
        [
            ...
            {adm_acl, [
                {'ensure_permission', 3, fun(_State, <<"settings:ddl:delete">>, Req) ->
