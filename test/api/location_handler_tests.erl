...
mock_request() ->
    #{method => <<"POST">>, path => <<"/api/v1/location">>}.
...
init_false_action_passthrough_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Req = mock_request(),
            {ok, RespReq, State} = location_handler:init(Req, #{
...
init_make_myself_visible_success_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Req = mock_request(),
            {ok, RespReq, _State} = location_handler:init(Req, #{
                action => make_myself_visible, current_uid => 12345
            }),
...
init_make_myself_unvisible_success_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Req = mock_request(),
            {ok, RespReq, _State} = location_handler:init(Req, #{
                action => make_myself_unvisible, current_uid => 12345
            }),
            ?assertEqual(200, maps:get(response_status, RespReq)),
            ?assertEqual(#{}, maps:get(payload, RespReq))
        end
...
init_people_nearby_success_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Req = mock_request(),
            {ok, RespReq, _State} = location_handler:init(Req, #{
                action => people_nearby, current_uid => 12345
            }),
