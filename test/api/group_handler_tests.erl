        ...
        fun() ->
            ...
            receive
                {page_query, Tb, Columns, Where, Order, Page, Size} ->
                    ?assertEqual(
                        <<"public.group g LEFT JOIN public.group_member m ON g.id = m.group_id">>,
                        Tb
                    ),
        ...
        fun() ->
            ...
            receive
                {msg_query, Tb, Where, Page, Size} ->
                    ...
                    %% msg_c2g 的群列是 to_id（历史缺陷：曾误用不存在的 to_groupid
                    %% 导致 SQL 失败被吞、API 恒 total=0）
                    ?assertEqual(#{to_id => 101}, Where),
                    ?assertEqual(1, Page),
                    ?assertEqual(20, Size)
            after 1000 ->
                ?assert(false)
