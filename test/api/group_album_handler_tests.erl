...
delete_album_not_found_maps_error_code_test_() ->
    ?WITH_MECKS([
        ...
        Result = group_album_handler:handle_action(delete_album, req_mock(), #{current_uid => 100}),
...
delete_album_permission_denied_maps_error_code_test_() ->
    ?WITH_MECKS([
        ...
        Result = group_album_handler:handle_action(delete_album, req_mock(), #{current_uid => 101}),
        ?assertEqual(req_error, Result),
...
upload_photo_with_json_content_type_test_() ->
    ?WITH_MECKS([
        ...
        {group_album_logic, [
            {'upload_photo', 5, fun(<<"group_1">>, 102, <<"album_3">>, <<"hello">>, <<"a.jpg">>) ->
                {ok, #{<<"photo_id">> => 1}}
            end}
        ]},
        {elib_response, [
        ...
        Result = group_album_handler:handle_action(upload_photo, req_mock(), #{current_uid => 102}),
        ?assertEqual(req_ok, Result)
    end).
...
upload_photo_json_album_not_found_maps_error_code_test_() ->
    ?WITH_MECKS([
        ...
        {group_album_logic, [
            {'upload_photo', 5, fun(<<"group_1">>, 102, <<"album_404">>, <<"hello">>, <<"a.jpg">>) ->
                {error, <<"相册不存在"/utf8>>}
            end}
        ]},
        {elib_response, [
...
upload_photo_json_permission_denied_maps_error_code_test_() ->
    ?WITH_MECKS([
        ...
        {group_album_logic, [
            {'upload_photo', 5, fun(<<"group_1">>, 102, <<"album_perm">>, <<"hello">>, <<"a.jpg">>) ->
                {error, <<"相册权限不足"/utf8>>}
            end}
        ]},
        {elib_response, [
...
upload_photo_with_malformed_multipart_returns_bad_request_test_() ->
    ?WITH_MECKS([
        ...
        Result = group_album_handler:handle_action(upload_photo, req_mock(), #{current_uid => 103}),
        ?assertEqual(req_error, Result),
        ?assertEqual(?ERR_BAD_REQUEST, receive_resp_code())
    end).
...
create_album_missing_gid_returns_bad_request_test_() ->
    ?WITH_MECKS([
        ...
        Result = group_album_handler:handle_action(create_album, req_mock(), #{current_uid => 105}),
        ?assertEqual(req_error, Result),
        ?assertEqual(?ERR_BAD_REQUEST, receive_resp_code())
    end).
...
list_albums_missing_gid_returns_bad_request_test_() ->
    ?WITH_MECKS([
        ...
        Result = group_album_handler:handle_action(list_albums, req_mock(), #{current_uid => 105}),
        ?assertEqual(req_error, Result),
        ?assertEqual(?ERR_BAD_REQUEST, receive_resp_code())
    end).
