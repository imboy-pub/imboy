...
whitelist_passport_test_() ->
    ?WITH_MECKS([
        {cowboy_req, [
            {'path', 1, fun(_Req) -> <<"/v1/passport/login">> end}
        ]}
    ], fun() ->
        Req = fake_req,
        Env = #{handler_opts => #{}},
...
normal_uid_request_pass_test_() ->
    ?WITH_MECKS([
        {cowboy_req, [
            {'path', 1, fun(_Req) -> <<"/v1/user/info">> end}
        ]},
        {throttle, [
...
normal_ip_request_pass_test_() ->
    ?WITH_MECKS([
        {cowboy_req, [
            {'path', 1, fun(_Req) -> <<"/v1/app_version/check">> end}
        ]},
        {elib_req, [
...
uid_vs_ip_selection_uid_test_() ->
    ?WITH_MECKS([
        {cowboy_req, [
            {'path', 1, fun(_Req) -> <<"/v1/msg/send">> end}
        ]},
        {throttle, [
...
uid_vs_ip_selection_ip_test_() ->
    ?WITH_MECKS([
        {cowboy_req, [
            {'path', 1, fun(_Req) -> <<"/v1/open/resource">> end}
        ]},
        {elib_req, [
