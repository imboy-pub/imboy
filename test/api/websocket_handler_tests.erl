...
init_stores_protocol_json_test_() ->
    ?WITH_MECKS([
        ...
        {websocket_ds, [
            {'check_subprotocols', 2, fun([<<"text">>], _Req) -> {cowboy_websocket, req1, s, o} end},
...
init_stores_protocol_protobuf_test_() ->
    ?WITH_MECKS([
        ...
        {websocket_ds, [
            ...
            {'select_subprotocol', 1, fun([<<"imboy-protobuf">>, <<"text">>]) -> <<"imboy-protobuf">> end},
            {'auth', 4, fun(_Auth, req1, State1, _Opt0) ->
...
websocket_init_error_always_json_test_() ->
    ?WITH_MECKS([
        ...
        %% 连接级错误始终使用 JSON text frame，即使协议为 protobuf
...
websocket_info_reply_map_protobuf_test_() ->
    ?WITH_MECKS(log_mocks(), fun() ->
        ...
        {reply, {binary, Bin}, _State2, hibernate} = websocket_handler:websocket_info({reply, Msg}, State),
...
websocket_info_reply_map_json_test_() ->
    ?WITH_MECKS(log_mocks(), fun() ->
        State = #{protocol => json},
        Msg = #{<<"code">> => 200, <<"msg">> => <<"ok">>},
        {reply, {text, Bin}, _State2, hibernate} = websocket_handler:websocket_info({reply, Msg}, State),
        Decoded = jsone:decode(Bin, [{object_format, map}]),
...
encode_delivery_frame_json_passthrough_test_() ->
    ?WITH_MECKS(log_mocks(), fun() ->
        ...
        State = #{protocol => json},
        {reply, {text, Bin}, _, hibernate} = websocket_handler:websocket_info({reply, JsonMsg}, State),
...
encode_delivery_frame_protobuf_reencode_test_() ->
    ?WITH_MECKS(log_mocks(), fun() ->
        ...
        State = #{protocol => protobuf},
        {reply, {binary, PbBin}, _, hibernate} = websocket_handler:websocket_info({reply, JsonMsg}, State),
...
protobuf_client_ack_success_test_() ->
    ?WITH_MECKS(log_mocks() ++ [
        ...
        AckPayload = imboy_pb:encode_msg(
            ...
            'PayloadClientAck'),
        PbMsg = imboy_codec:encode(protobuf,
            #{<<"id">> => <<"msg_pb_1">>, <<"type">> => <<"CLIENT_ACK">>,
              <<"msg_type">> => <<"client_ack">>, <<"payload">> => AckPayload}),
