...
mock_request() ->
    #{method => <<"POST">>, path => <<"/v1/user/device">>}.

init_false_action_passthrough_test_() ->
    ?TEST_SIMPLE(fun() ->
        Req = mock_request(),
        {ok, RespReq, State} = user_device_handler:init(Req, #{action => false, current_uid => 12345}),
...
init_page_success_test_() ->
    ?WITH_MECKS([
        ...
        Req = mock_request(),
        {ok, RespReq, _State} = user_device_handler:init(Req, #{action => page, current_uid => 12345}),
...
init_change_name_success_test_() ->
    ?WITH_MECKS([
        ...
        Req = mock_request(),
        {ok, RespReq, _State} = user_device_handler:init(Req, #{action => change_name, current_uid => 12345}),
        ?assertEqual(200, maps:get(response_status, RespReq))
    end).
...
init_delete_success_test_() ->
    ?WITH_MECKS([
        ...
        Req = mock_request(),
        {ok, RespReq, _State} = user_device_handler:init(Req, #{action => delete, current_uid => 12345}),
        ?assertEqual(200, maps:get(response_status, RespReq))
    end).
...
init_check_login_no_conflict_test_() ->
    ?WITH_MECKS([
        ...
        Req = mock_request(),
        {ok, RespReq, _State} = user_device_handler:init(Req, #{action => check_login, current_uid => 12345}),
        Payload = maps:get(payload, RespReq),
        ?assertEqual(200, maps:get(response_status, RespReq)),
