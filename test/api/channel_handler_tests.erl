...
accept_invitation_handler_logic_ds_chain_already_accepted_is_success_and_silent_test_() ->
    ?WITH_MECKS(
        [
            ...
            {channel_invitation_repo, [
                {'find_by_id', 1, fun(_) ->
                    erlang:error(should_not_call_find_by_id_when_already_accepted)
                end}
            ]},
            {msg_s2c_ds, [
                {'send', 7, fun(_, _, _, _, _, _, _) ->
                    erlang:error(should_not_send_duplicate_invitation_notify)
                end}
            ]},
            {elib_response, [
                {'success', 2, fun(_Req, Payload) -> {ok_resp, Payload} end},
        ...
        [
            ...
            {channel_order_repo, [
                {'find_by_order_no', 1, fun(_) ->
                    erlang:error(should_not_call_find_by_order_no)
                end}
            ]},
            {elib_response, [
                {'error', 2, fun(_Req, Msg) -> {error_resp, Msg} end}
        ...
        [
            ...
            {channel_repo, [
                {'increment_subscribers', 3, fun(_, _, _) ->
                    erlang:error(should_not_call_increment)
                end}
            ]},
            {elib_response, [
                {'error', 2, fun(_Req, Msg) -> {error_resp, Msg} end}
...
update_handler_logic_repo_chain_notify_crash_still_returns_success_test_() ->
    ...
    ?WITH_MECKS(
        [
            ...
            {msg_s2c_ds, [
                {'send', 7, fun(
                    0, [1001, 2002], <<"channel_updated">>, <<>>, null, _Payload, no_save
                ) ->
                    erlang:error(mock_notify_crash)
                end}
            ]},
...
delete_message_handler_logic_repo_chain_notify_crash_still_returns_success_test_() ->
    ...
    ?WITH_MECKS(
        [
            ...
            {msg_s2c_ds, [
                {'send', 7, fun(
                    0, [1001, 2002], <<"channel_message_deleted">>, <<>>, null, _Payload, save
                ) ->
                    erlang:error(mock_notify_crash)
                end}
            ]},
