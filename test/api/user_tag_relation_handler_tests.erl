...
mock_request() ->
    #{method => <<"POST">>, path => <<"/api/v1/user/tag/relation">>}.

init_false_action_passthrough_test_() ->
    ?TEST_SIMPLE(fun() ->
        Req = mock_request(),
        {ok, RespReq, State} = user_tag_relation_handler:init(Req, #{
...
init_add_collect_success_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Req = mock_request(),
            {ok, RespReq, _State} = user_tag_relation_handler:init(Req, #{
...
init_friend_page_success_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            Req = mock_request(),
            {ok, RespReq, _State} = user_tag_relation_handler:init(Req, #{
                action => friend_page, current_uid => 12345
            }),
