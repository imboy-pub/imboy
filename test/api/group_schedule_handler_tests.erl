...
init_create_missing_group_id_returns_bad_request_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, Req1, _State} = group_schedule_handler:init(req_mock(), #{
                action => create, current_uid => 300
            }),
    ...
    ?WITH_MECKS(
        feature_gate_bypass_mocks() ++
            [
                ...
                {group_schedule_logic, [
                    {'create_schedule', 9, fun(
                        9, 300, _Title, _Desc, _Loc, StartAt, EndAt, _Remind, _Participants
                    ) ->
    ...
    ?WITH_MECKS(
        feature_gate_bypass_mocks() ++
            [
                ...
                {group_schedule_logic, [
                    {'update_schedule', 7, fun(
                        _ScheduleId, _Uid, _Title, _Desc, _Loc, _StartAt, _EndAt
                    ) ->
                        {error, unauthorized}
                    end}
                ]},
    ...
    ?WITH_MECKS(
        feature_gate_bypass_mocks() ++
            [
                ...
                {group_schedule_logic, [
                    {'update_schedule', 7, fun(
                        <<"sched_77">>, 300, _Title, _Desc, _Loc, StartAt, EndAt
                    ) ->
                        ?assert(is_binary(StartAt)),
                        ?assert(is_binary(EndAt)),
                        ok
...
init_confirm_participant_not_found_returns_forbidden_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, Req1, _State} = group_schedule_handler:init(req_mock(), #{
                action => confirm, current_uid => 300
            }),
            ?assertEqual(req_error, Req1),
            ?assertEqual(?ERR_FORBIDDEN, receive_resp_code())
        end
...
init_my_list_with_time_range_query_compatible_test_() ->
    ?WITH_MECKS(
        ...
        fun() ->
            {ok, Req1, _State} = group_schedule_handler:init(req_mock(), #{
                action => my_list, current_uid => 300
            }),
            ?assertEqual(req_ok, Req1)
        end
    ).
