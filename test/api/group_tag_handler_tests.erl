...
init_add_missing_gid_error_test_() ->
    ?WITH_MECKS([
        ...
        Req = mock_request(),
        {ok, RespReq, _State} = group_tag_handler:init(Req, #{action => add, current_uid => 12345}),
...
init_remove_success_test_() ->
    ?WITH_MECKS([
        ...
        Req = mock_request(),
        {ok, RespReq, _State} = group_tag_handler:init(Req, #{action => remove, current_uid => 12345}),
        ?assertEqual(200, maps:get(response_status, RespReq)),
...
init_list_success_test_() ->
    ?WITH_MECKS([
        ...
        Req = mock_request(),
        {ok, RespReq, _State} = group_tag_handler:init(Req, #{action => list, current_uid => 12345}),
        ?assertEqual(200, maps:get(response_status, RespReq)),
...
init_search_success_test_() ->
    ?WITH_MECKS([
        ...
        Req = mock_request(),
        {ok, RespReq, _State} = group_tag_handler:init(Req, #{action => search, current_uid => 12345}),
        ?assertEqual(200, maps:get(response_status, RespReq)),
        Payload = maps:get(payload, RespReq),
...
init_hot_default_limit_success_test_() ->
    ?WITH_MECKS([
        ...
        Req = mock_request(),
        {ok, RespReq, _State} = group_tag_handler:init(Req, #{action => hot, current_uid => 12345}),
        ?assertEqual(200, maps:get(response_status, RespReq)),
        Payload = maps:get(payload, RespReq),
