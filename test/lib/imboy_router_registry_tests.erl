...
register_then_lookup_test_() ->
    {setup, fun setup/0, fun cleanup/1,
     fun(_Pid) ->
         R1 = valid_route(channel, <<"/v1/channel/x">>),
         R2 = valid_route(channel, <<"/v1/channel/y">>),
...
register_overwrites_existing_test_() ->
    {setup, fun setup/0, fun cleanup/1,
     fun(_Pid) ->
         OldR = valid_route(channel, <<"/v1/channel/old">>),
         NewR = valid_route(channel, <<"/v1/channel/new">>),
...
all_routes_flattens_test_() ->
    {setup, fun setup/0, fun cleanup/1,
     fun(_Pid) ->
         RChan = valid_route(channel, <<"/v1/channel/x">>),
         RMom = valid_route(moment, <<"/v1/moment/y">>),
...
plugin_names_lists_registered_test_() ->
    {setup, fun setup/0, fun cleanup/1,
     fun(_Pid) ->
         ok = imboy_router_registry:register(channel, [valid_route(channel, <<"/v1/channel/x">>)]),
         ok = imboy_router_registry:register(moment, [valid_route(moment, <<"/v1/moment/y">>)]),
         ?_assertEqual(
...
register_rejects_invalid_namespace_test_() ->
    {setup, fun setup/0, fun cleanup/1,
     fun(_Pid) ->
         ...
         ?_assertMatch({error, {invalid_route_namespace, <<"/v1/wrong/x">>}},
...
register_rejects_malformed_spec_test_() ->
    {setup, fun setup/0, fun cleanup/1,
     fun(_Pid) ->
         BadSpec = not_a_map,
         ?_assertMatch({error, {invalid_route_spec, _}},
