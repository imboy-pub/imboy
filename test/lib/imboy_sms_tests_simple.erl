...
-define(TEST_CONTENT, <<"验证码是1234"/utf8>>).
...
sms_content_format_test_() ->
    ?_test(fun() ->
        ...
        VerificationFormats = [
            <<"验证码是1234"/utf8>>,
            <<"您的验证码是：5678，有效期5分钟"/utf8>>,
            <<"【IMBoy】验证码：9012，请勿泄露"/utf8>>,
            <<"Code: 3456, valid for 5 minutes">>,
            <<"您的动态密码是7890，如非本人操作请忽略"/utf8>>,
            <<"【平台】123456，登录验证码，5分钟内有效"/utf8>>
        ],
...
sms_response_format_test_() ->
    ?_test(fun() ->
        ...
        % 测试错误响应格式
        ErrorResponses = [
            #{<<"code">> => 1, <<"message">> => <<"账号名为空"/utf8>>},
            #{<<"code">> => 2, <<"message">> => <<"密码错误"/utf8>>},
            #{<<"code">> => 3, <<"message">> => <<"手机号格式错误"/utf8>>},
            #{<<"code">> => 4, <<"message">> => <<"内容包含敏感词"/utf8>>},
            #{<<"code">> => 5, <<"message">> => <<"余额不足"/utf8>>}
        ],
