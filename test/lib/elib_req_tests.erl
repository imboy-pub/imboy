-module(elib_req_tests).
-include_lib("eunit/include/eunit.hrl").
-include("eunit_setup.hrl").

%%%===================================================================
%%% @doc
%%% elib_req 模块的 EUnit 测试
%%%
%%% 目标：验证请求处理工具功能
...
peer_ip_ipv4_test_() ->
    ?TEST_WITH_APP(fun() ->
        ...
        try
            ...
            ?assertEqual(<<"192.168.1.100">>, elib_req:peer_ip(Req0)),
            ?assert(meck:validate(cowboy_req))
        after
            meck:unload(cowboy_req)
        end
...
cookie_empty_cookies_test_() ->
    ?TEST_WITH_APP(fun() ->
        ...
        try
            ...
            ?assertEqual(false, elib_req:cookie(<<"any">>, Req0)),
            ?assert(meck:validate(cowboy_req))
        after
            meck:unload(cowboy_req)
        end
...
parse_json_body_invalid_test_() ->
    ?TEST_SIMPLE(fun() ->
        Body = <<"{invalid json}">>,
        Result = elib_req:parse_json_body(Body),
