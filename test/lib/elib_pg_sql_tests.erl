-module(elib_pg_sql_tests).
-include_lib("eunit/include/eunit.hrl").
-include("eunit_setup.hrl").

%%%===================================================================
%%% @doc
%%% elib_pg_sql 模块的 EUnit 测试
%%%
%%% 目标：验证 SQL 构造函数的正确性
...
insert_with_binary_key_test_() ->
    ?TEST_WITH_DB(fun() ->
        Table = <<"user">>,
        Map = #{<<"user_id">> => 123},
        {_Sql, Params} = elib_pg_sql:insert(Table, Map),
...
update_single_field_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        Map = #{name => <<"Charlie">>},
        WhereSql = <<"id = $1">>,
        {Sql, Params} = elib_pg_sql:update(Table, Map, WhereSql, []),
        SqlBin = iolist_to_binary(Sql),
...
update_empty_map_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        Map = #{},
        WhereSql = <<"id = 1">>,
        {_Sql, Params} = elib_pg_sql:update(Table, Map, WhereSql, []),
        ?assertEqual([], Params)
    end).
...
update_4_single_where_param_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        ?assertMatch(
            {_, _}, binary:match(SqlBin, <<"UPDATE user SET name = $2,age = $3 WHERE id = $1">>)
        ),
...
update_4_multiple_where_params_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        ?assertMatch(
            {_, _},
            binary:match(SqlBin, <<"UPDATE user SET status = $3 WHERE id = $1 AND status = $2">>)
        ),
        % 参数顺序：WHERE 参数在前，SET 参数在后
...
update_4_complex_where_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        {Sql, Params} = elib_pg_sql:update(Table, Map, WhereSql, WhereParams),
        SqlBin = iolist_to_binary(Sql),
        % 套件隔离治理：SET 子句内列的先后取决于 maps:to_list 的迭代序，
        % 不对整段 SQL 做字面断言；改为逐段钉死契约：
        % 1) 前缀与 WHERE 子句原样保留；
        ?assertMatch({_, _}, binary:match(SqlBin, <<"UPDATE user SET">>)),
        ?assertMatch(
            {_, _},
            binary:match(SqlBin, <<"WHERE id = $1 OR (status = $2 AND created_at > $3)">>)
        ),
        % 2) WHERE 参数 $1..$3 不被改写，SET 占位符从 WHERE 参数数量+1（$4）起；
        ?assertMatch({_, _}, binary:match(SqlBin, <<"= $4">>)),
...
update_4_with_in_clause_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        SqlBin = iolist_to_binary(Sql),
        ?assertMatch(
            {_, _}, binary:match(SqlBin, <<"UPDATE user SET status = $4 WHERE id IN ($1, $2, $3)">>)
        ),
...
insert_batch_single_row_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        Cols = [id],
        Rows = [[1]],
        {_Sql, Params} = elib_pg_sql:insert_batch(Table, Cols, Rows),
...
build_select_all_fields_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        Where = #{},
        Opts = #{},
        {_Sql, Params} = elib_pg_sql:build_select(Table, Fields, Where, Opts),
        ?assertEqual([], Params)
    end).
...
build_select_specific_fields_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        Where = #{id => 1},
        Opts = #{},
        {Sql, Params} = elib_pg_sql:build_select(Table, Fields, Where, Opts),
        SqlBin = iolist_to_binary(Sql),
...
build_where_mixed_conditions_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        ?assertMatch(
            {_, _}, binary:match(Sql, <<"WHERE id IN ($1,$2) AND name = $3 AND status > $4">>)
        ),
...
build_where_with_complex_nested_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        ?assertMatch(
            {_, _}, binary:match(Sql, <<" OR ">>, [{scope, {Pos1 + 4, byte_size(Sql) - Pos1 - 4}}])
        ),
        % 验证参数集合正确（排序后比较）
...
page_basic_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        ?assertMatch(
            {_, _},
            binary:match(
                SqlBin, <<"SELECT * FROM user WHERE id > 0 ORDER BY id DESC LIMIT $1 OFFSET $2">>
            )
        ),
