...
sha256_test_() ->
    ?TEST_SIMPLE(fun() ->
        Input = <<"test">>,
        Result = elib_hasher:sha256(Input),
        % 验证SHA256哈希结果长度
...
hmac_sha256_test_() ->
    ?TEST_SIMPLE(fun() ->
        ...
        ?assertEqual(64, byte_size(Result)),
        % 验证相同输入和密钥产生相同输出
        Result2 = elib_hasher:hmac_sha256(Input, Key),
        ?assertEqual(Result, Result2)
    end).
...
password_hash_test_() ->
    ?TEST_SIMPLE(fun() ->
        ...
        Salt = <<"salt">>,
        Result = elib_hasher:password_hash(Password, Salt),
        % 验证密码哈希结果
        ?assertMatch(<<_/binary>>, Result),
        ?assertNotEqual(<<>>, Result),
        % 验证包含盐值影响
        Result2 = elib_hasher:password_hash(Password, <<"different">>),
...
password_verify_test_() ->
    ?TEST_SIMPLE(fun() ->
        Password = <<"password123">>,
        Salt = <<"salt">>,
        Hash = elib_hasher:password_hash(Password, Salt),
        % 验证密码验证功能
        ?assert(elib_hasher:password_verify(Password, Salt, Hash)),
        ?assertNot(elib_hasher:password_verify(<<"wrong">>, Salt, Hash))
    end).
