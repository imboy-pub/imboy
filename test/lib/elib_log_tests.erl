...
%% 测试debug级别日志
%% ?LOG_LEVEL=info（elib_log 编译期阈值）：debug 低于阈值被运行时过滤，
%% 不触达 lager；相邻阈值 info 应触达并带全 metadata
debug_log_test_() ->
    ?TEST_WITH_APP(fun() ->
        % 设置Mock
        ...
        try
            % debug 低于 ?LOG_LEVEL=info 阈值：应被过滤
            elib_log:debug(?TEST_MESSAGE),
            ?assertEqual(0, meck:num_calls(lager, log, 3)),

            % info 达到阈值：应触达 lager
            elib_log:info(?TEST_MESSAGE),
            ?assertEqual(1, meck:num_calls(lager, log, 3))
        after
            % 清理Mock
            meck:unload(lager)
        end
...
internal_log_functions_test_() ->
    ?TEST_WITH_APP(fun() ->
        ...
        % Mock lager:log/3（?LOG_LEVEL=info，用 info 验证触达路径）
        meck:expect(lager, log, 3, fun(Level, Metadata, Message) ->
            ?assertEqual(info, Level),
            ?assertMatch([_ | _], Metadata),
            ?assertMatch([_ | _], Message)
...
log_level_filtering_test_() ->
    ?TEST_WITH_APP(fun() ->
        ...
        try
            ...
            % 当前实现的日志级别阈值是 info（elib_log ?LOG_LEVEL 编译期宏）：
            % debug 被过滤，其余 4 个级别触达 lager
