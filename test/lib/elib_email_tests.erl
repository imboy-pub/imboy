...
send_with_subject_only_test_() ->
    ?WITH_MECKS(
        [
            ...
            {ec_cnv, [
                {'to_binary', 1, fun
                    (Input) when is_list(Input) -> list_to_binary(Input);
                    (Input) -> Input
                end}
            ]}
        ],
                ...
                {'encode', 1, fun(Email) ->
                    case Email of
                        {<<"text">>, <<"html">>, EmailHeaders, _Params, _Body} ->
                            ?assertMatch(
                                {<<"From">>, _}, lists:keyfind(<<"From">>, 1, EmailHeaders)
                            ),
                            ?assertMatch({<<"To">>, _}, lists:keyfind(<<"To">>, 1, EmailHeaders)),
                            ?assertMatch(
                                {<<"Subject">>, _}, lists:keyfind(<<"Subject">>, 1, EmailHeaders)
                            );
                        _ ->
        ...
        [
            ...
            {mimemail, [
                {'encode', 1, fun(_Email) ->
                    erlang:error(
                        badmatch,
                        {error, {999999, smtp_rfc5322_parse, ["syntax error before: ", []]}}
                    )
                end}
            ]}
        ],
