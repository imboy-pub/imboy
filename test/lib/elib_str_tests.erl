-module(elib_str_tests).
-include_lib("eunit/include/eunit.hrl").
-include("eunit_setup.hrl").

%%%===================================================================
%%% @doc
%%% elib_str 模块的 EUnit 测试
%%%
%%% 目标：验证字符串工具功能
...
startswith_binary_match_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = elib_str:startswith(<<"abc">>, <<"abcdef">>),
...
startswith_binary_no_match_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = elib_str:startswith(<<"xyz">>, <<"abcdef">>),
...
endswith_binary_match_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = elib_str:endswith(<<"ing">>, <<"This is the end of the binary">>),
        ?assertEqual(false, Result)
    end).
...
endswith_binary_actual_match_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = elib_str:endswith(<<"binary">>, <<"This is the end of the binary">>),
        ?assertEqual(true, Result)
    end).
...
endswith_string_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = elib_str:endswith("string", "This is the end of the string"),
        ?assertEqual(true, Result)
    end).
...
endswith_suffix_longer_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = elib_str:endswith(<<"long suffix">>, <<"short">>),
        ?assertEqual(false, Result)
    end).
...
replace_basic_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = elib_str:replace(<<"Hello, old world!">>, <<"old">>, <<"new">>),
...
replace_no_match_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = elib_str:replace(<<"Hello world">>, <<"xyz">>, <<"new">>),
...
replace_single_quote_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = elib_str:replace_single_quote(<<"hello D'l">>),
...
replace_multiple_quotes_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = elib_str:replace_single_quote(<<"'hello' 'world'">>),
        ?assertEqual(<<"\\'hello\\' \\'world\\'">>, Result)
    end).
...
trunc_binary_need_trunc_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = elib_str:trunc(<<"12345678">>, 7),
