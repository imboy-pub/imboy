...
success_test() ->
    ...
    ?assertEqual({ok, success_result}, imboy_retry:with_retry(Fun)).
...
retry_success_test() ->
    ...
    Result = imboy_retry:with_retry(Fun, 5, 10),
...
retry_failed_test() ->
    ...
    Result = imboy_retry:with_retry(Fun, 2, 10),
...
fixed_backoff_test() ->
    ...
    Result = imboy_retry:with_retry(Fun, 3, 50, fixed),
...
linear_backoff_test() ->
    ...
    ?assertEqual({ok, success}, imboy_retry:with_retry(Fun, 2, 10, linear)).
...
exponential_backoff_test() ->
    ...
    ?assertEqual({ok, success}, imboy_retry:with_retry(Fun, 2, 10, exponential)).
...
different_error_types_test() ->
    ...
    lists:foreach(fun({Fun, _ExpectedError}) ->
        % 应该重试后失败
        Result = imboy_retry:with_retry(Fun, 1, 10),
        ?assertEqual({error, max_retries_exceeded}, Result)
    end, TestCases).
...
return_value_test() ->
    Fun = fun() -> 42 end,
    ?assertEqual({ok, 42}, imboy_retry:with_retry(Fun)).
...
complex_return_value_test() ->
    ...
    {ok, Result} = imboy_retry:with_retry(Fun),
...
timeout_success_test() ->
    ...
    ?assertEqual({ok, success},
                 imboy_retry:with_retry_and_timeout(Fun, 200, 1)).
...
timeout_and_retry_test() ->
    ...
    Result = imboy_retry:with_retry_and_timeout(Fun, 100, 3, 50),
    ?assertEqual({ok, success}, Result),
     ...
     fun(_) ->
         [
          ?_test(begin
              ...
              Result = imboy_retry:with_retry(DbFun, 5, 10),
     ...
     fun(_) ->
         [
          ?_test(begin
              ...
              Result = imboy_retry:with_retry(ApiFun, 3, 100),
