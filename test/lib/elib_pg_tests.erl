...
pluck_existing_value_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        Field = <<"id">>,
        Result = elib_pg:pluck(Table, Field, #{id => 1}, #{}, 0),
        case Result of
...
pluck_with_opts_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        Result = elib_pg:pluck(Table, Field, #{id => 1}, Opts, 0),
...
pluck_value_success_test_() ->
    ?TEST_WITH_DB(fun() ->
        Table = user_repo:tablename(),
        Field = <<"id">>,
        Result = elib_pg:pluck_value(Table, Field, #{id => 1}, #{}, 0),
...
pluck_value_with_opts_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        Field = <<"id">>,
        Opts = #{order_by => [{id, desc}], limit => 1},
        Result = elib_pg:pluck_value(Table, Field, #{id => 1}, Opts, 0),
        % 应该返回实际的 ID 值
        ?assert(is_integer(Result), "Expected integer value")
    end).
...
insert_with_timeout_test_() ->
    ?TEST_WITH_DB(fun() ->
        Table = user_repo:tablename(),
        Data = #{account => <<"test_timeout">>, created_at => elib_dt:now()},
        Result = elib_pg:insert(Table, Data),
        case Result of
            {ok, AffectedCount} when is_integer(AffectedCount) -> ?assert(AffectedCount >= 0);
            {ok, _} -> ?assert(true);
...
page_basic_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        Page = 1,
        Size = 10,
        Result = elib_pg:page(Table, WhereMap, Page, Size),
        case Result of
...
page_with_column_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        Page = 1,
        Size = 5,
        Result = elib_pg:page(Table, Column, WhereMap, OrderBy, Page, Size),
        case Result of
            {ok, PageData} when is_map(PageData); is_list(PageData) -> ?assert(true);
            _ -> ?assert(false, "Expected {ok, PageData}")
        end
...
page_with_timeout_test_() ->
    ?TEST_WITH_DB(fun() ->
        ...
        Page = 1,
        Size = 10,
        Result = elib_pg:page(Table, <<"*">>, WhereMap, OrderBy, Page, Size),
        case Result of
            {ok, PageData} when is_map(PageData); is_list(PageData) -> ?assert(true);
            _ -> ?assert(false, "Expected {ok, PageData}")
        end
