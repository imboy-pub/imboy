...
json_maybe_object_test_() ->
    ?TEST_SIMPLE(fun() ->
        ...
        Result = imboy_cnv:json_maybe(Json),
...
json_maybe_non_json_test_() ->
    ?TEST_SIMPLE(fun() ->
        Val = <<"not a json">>,
        Result = imboy_cnv:json_maybe(Val),
...
json_maybe_utf8_chinese_test_() ->
    ?TEST_SIMPLE(fun() ->
        ...
        Result = imboy_cnv:json_maybe(Json),
        ?assertMatch(#{<<"name">> := <<"张三">>, <<"city">> := <<"北京">>}, Result)
    end).

%% @doc 测试 UTF-8 emoji 支持
json_maybe_utf8_emoji_test_() ->
    ?TEST_SIMPLE(fun() ->
        Json = <<"{\"message\":\"你好 👋\",\"emoji\":\"😀\"}">>,
        Result = imboy_cnv:json_maybe(Json),
        ?assertMatch(#{<<"message">> := <<"你好 👋">>, <<"emoji">> := <<"😀">>}, Result)
    end).

%% @doc 测试 UTF-8 混合内容
json_maybe_utf8_mixed_test_() ->
    ?TEST_SIMPLE(fun() ->
        Json = <<"{\"zh\":\"中文\",\"en\":\"English\",\"emoji\":\"🌍\"}">>,
        Result = imboy_cnv:json_maybe(Json),
        ?assertMatch(#{<<"zh">> := <<"中文">>, <<"en">> := <<"English">>, <<"emoji">> := <<"🌍">>}, Result)
    end).
...
implode_binary_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:implode(",", [<<"a">>, <<"b">>, <<"c">>]),
...
implode_integer_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:implode(",", [1, 2, 3]),
        ?assertEqual(<<"1,2,3">>, Result)
    end).
...
implode_float_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:implode(",", [1, 2, 3.3]),
...
implode_integer_separator_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:implode(44, [<<"a">>, <<"b">>]),
        ?assertEqual(<<"a,b">>, Result)
    end).
...
remove_dups_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:remove_dups([1, 2, 2, 3, 3, 3]),
        ?assertEqual([1, 2, 3], Result)
    end).
...
remove_dups_no_dups_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:remove_dups([1, 2, 3]),
        ?assertEqual([1, 2, 3], Result)
    end).
...
remove_dups_string_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:remove_dups([<<"a">>, <<"b">>, <<"a">>, <<"c">>]),
        ?assertEqual([<<"a">>, <<"b">>, <<"c">>], Result)
    end).
...
vsn_major_full_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:vsn_major("1.2.3"),
...
vsn_major_two_parts_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:vsn_major("2.3"),
...
vsn_major_prerelease_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:vsn_major("3.0.0-alpha"),
...
vsn_major_build_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:vsn_major("1.2.3+build123"),
        ?assertEqual(<<"1">>, Result)
    end).
...
map_to_query_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:map_to_query(#{a => 1, b => 2, c => 3}),
        % 注意：map 的顺序可能不同
...
map_to_query_single_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:map_to_query(#{key => <<"value">>}),
...
list_to_binary_string_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:list_to_binary_string([513251, 62829, 62825]),
...
list_to_binary_string_single_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:list_to_binary_string([123]),
...
list_to_binary_string_empty_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:list_to_binary_string([]),
        ?assertEqual(<<>>, Result)
    end).
...
safe_to_binary_string_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:safe_to_binary("hello"),
...
safe_to_binary_integer_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:safe_to_binary(123),
        ?assertEqual(<<"123">>, Result)
    end).
...
safe_to_binary_complex_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:safe_to_binary({error, reason, [details]}),
        ?assert(is_binary(Result)),
...
safe_to_binary_map_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:safe_to_binary(#{key => <<"value">>}),
        ?assert(is_binary(Result)),
        ?assertNotEqual(<<>>, Result)
    end).
...
convert_at_timestamps_empty_list_test_() ->
    ?TEST_SIMPLE(fun() ->
        Result = imboy_cnv:convert_at_timestamps([]),
        ?assertEqual([], Result)
    end).
...
convert_at_timestamps_at_field_test_() ->
    ?TEST_SIMPLE(fun() ->
        ...
        Result = imboy_cnv:convert_at_timestamps(Input),
