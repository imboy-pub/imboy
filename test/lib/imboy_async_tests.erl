...
async_retry_success_test() ->
    ...
    Pid = imboy_async:async_retry(Fun, 5, 10),
    ?assert(is_pid(Pid)),
...
async_retry_failed_test() ->
    ...
    Pid = imboy_async:async_retry(Fun, 2, 10),
    ?assert(is_pid(Pid)),
...
async_with_timeout_api_success_test() ->
    ...
    Pid = imboy_async:async_with_timeout(fun() ->
...
async_with_timeout_api_retry_test() ->
    ...
    Pid = imboy_async:async_with_timeout(Fun, 100),
    ?assert(is_pid(Pid)),
...
async_throw_different_exceptions_test_() ->
    ...
    lists:map(fun({Fun, ExpectedError}) ->
        ?_test(begin
            Parent = self(),
            imboy_async:async_with_callback(Fun, Parent),
