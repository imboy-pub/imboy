...
send_yjsms_test_() ->
    ?TEST_WITH_APP(fun() ->
        ...
        meck:expect(config_ds, env, fun
            (yjsms_account, _) -> <<"test_account">>;
            (yjsms_secret, _) -> <<"test_secret">>;
            (yjsms_url, _) -> <<"https://test.sms.api">>
        end),
        ...
        % Mock MD5计算
        meck:expect(elib_hasher, md5, fun
            (<<"test_secret">>) -> <<"hashed_secret">>;
            (_) -> <<"signature_hash">>
        end),
...
send_jsms_test_() ->
    ?TEST_WITH_APP(fun() ->
        ...
        try
            ...
            ?assertEqual(
                {ok, #{<<"msg_id">> => <<"123456789">>, <<"send_id">> => <<"987654321">>}}, Result
            ),
