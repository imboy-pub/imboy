...
-- 用户 ID 使用 9×10^18 高位偏移（BIGINT max ≈ 9.2×10^18），
-- TSID 碰撞地平线 ~2146 年（纪元 2025 + 121 年），在 TSID 139.5 年有效期内安全。

BEGIN;

-- 1. ark 默认 Agent
INSERT INTO public."user" (id, account, nickname, password, status, account_type, reg_ip, reg_cosv, created_at)
SELECT 9000000000000000001, 'agent_ark', 'Ark 助手', '', 1, 1, '127.0.0.1', 'migration', now()
...
SELECT 9000000000000000001, 'ark', 'doubao-lite-4k', '你是一个有帮助的AI助手，请专业、友善地回答用户问题。', 0, 1, 'Ark 平台 AI 助手', 1
WHERE NOT EXISTS (SELECT 1 FROM public.ai_agent WHERE provider = 'ark');

-- 2. bailian 默认 Agent
INSERT INTO public."user" (id, account, nickname, password, status, account_type, reg_ip, reg_cosv, created_at)
SELECT 9000000000000000002, 'agent_bailian', '百炼助手', '', 1, 1, '127.0.0.1', 'migration', now()
WHERE NOT EXISTS (SELECT 1 FROM public.ai_agent WHERE provider = 'bailian');

INSERT INTO public.ai_agent (user_id, provider, model, system_prompt, owner_uid, status, description, visibility)
SELECT 9000000000000000002, 'bailian', 'qwen3.7-flash', '你是一个有帮助的AI助手，请专业、友善地回答用户问题。', 0, 1, '百炼平台 AI 助手', 1
WHERE NOT EXISTS (SELECT 1 FROM public.ai_agent WHERE provider = 'bailian');
