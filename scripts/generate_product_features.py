...
# BUILD-00R 后端物理裁剪映射：未被选中的 feature，其专属模块经 erlang.mk 原生
# ERLC_EXCLUDE 不参与编译，并自动退出 .app modules 与 release 包。
# 运行时门（imboy_feature:compiled / compiled_routes / plugin_compiled）与
...
def render(contract: dict) -> dict[Path, str]:
    ...
    excluded_modules = sorted(
        module
        for feature, modules in FEATURE_BACKEND_MODULES.items()
    ...
    backend = (
        ...
        + "".join(
            "-define(IMBOY_FEATURE_" + feature.upper() + ", true).\n"
...
""".format(
    ...
    return {
        ...
        root / "include/generated/imboy_product_features_erlc.mk": erlc_mk,
